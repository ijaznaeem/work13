...
export class CashSaleComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  public calculation() {
    ...
    this.data.getAll().then((d) => {
      ...
      this.sale.Labour = FindTotal(d, 'Labour');
  ...
  Cancel() {
    ...
    this.sdetails.StoreID = localStorage.getItem('StoreID') || '1';

    this.StoreSelected({ StoreID: this.sdetails.StoreID, StoreName: '' });
    this.btnsave = false;
    this.isPosted = false;
    this.SelectCust = {};
  }
  ...
  Search(term = '') {
    ...
    this.bsModalRef.content.Event.subscribe((res) => {
      if (res.res == 'ok') {
        ...
        this.sale.OrderID = res.data.OrderID;
        this.LoadOrder(this.sale.OrderID);
      } else {
        this.bsModalRef?.hide();
  ...
  LoadOrder(code) {
    this.http.getData('orders/' + code).then((r: any) => {
      if (r) {
        this.sale.CustomerID = r.CustomerID;
        this.sale.CustomerName = r.CustomerName;
