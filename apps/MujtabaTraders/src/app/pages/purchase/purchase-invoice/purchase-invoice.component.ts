...
export class PurchaseInvoiceComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  private AddToDetails(ord: any) {
    const obj = {
      ...
      Expense: ord.Expense,
      ExpenseAmount: ord.Expense * ord.Qty,
      RateUnit: ord.RateUnit,
      Amount:
        (ord.PPrice * (ord.Qty * ord.Packing + ord.KGs * 1)) ,
  ...
  public onEdit(event) {
    ...
    event.newData.ExpenseAmount =
      (event.newData.Qty * event.newData.Expense ) ;
  ...
  public calculation() {
    ...
    this.data.getAll().then((d) => {
      this.purchase.Amount = FindTotal(d, 'Amount');
      this.purchase.FrieghtCharges = FindTotal(d, 'ExpenseAmount');
