  ...
  implements OnInit, OnChanges, AfterViewInit
{
  ...
  ngOnInit() {
    ...
    this.route.paramMap.subscribe((params) => {
      ...
      if (this.OrderID!= '') OrderID = this.OrderID

      this.http
        .getData('qryorders?filter=Orderid=' + OrderID)
        .then((inv: any) => {
          this.Order = inv[0];

          this.http
            .getData('qryorderdetails?filter=OrderID=' + OrderID)
            .then((r: any) => {
              let k = 0;
              r.forEach((e) => {
                e.Sno = ++k;
  ...
  FindTotal(fld) {
    if (this.Order.Detail) {
      return this.RoundIt(FindTotal(this.Order.Detail, fld), 0);
    } else {
      return 0;
    }
  }
  ...
  SaveAsPdf() {
    ...
    html2canvas(data).then((canvas) => {
      ...
      pdf.save(
        this.Order.CustomerName + '-' + this.Order.OrderID + '.pdf'
      ); // Generated PDF
    });
  }
