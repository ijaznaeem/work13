 ...
 public Typefields: Object = { text: 'AcctType', value: 'AcctTypeID' };
  ...
  ngOnInit() {
    ...
    this.http.getData('accttypes').then(res =>{
     ...
     LoadData() {
         ...
         if(this.CustomerID !== '' ){
          ...
          this.http.getData('qrypurchase?filter=date between "'+this.getDate(this.dteFilter.dteFrom)+'" and "'+this.getDate(this.dteFilter.dteTo)+'" and CustomerID="'+this.CustomerID+'" and Type =2').then(data => {
            this.data = data;
            console.log(this.data);
          })
         }else{
          this.http.getData('qrypurchase?filter=date between "'+this.getDate(this.dteFilter.dteFrom)+'" and "'+this.getDate(this.dteFilter.dteTo)+'" and Type =2').then(data => {
            this.data = data;
            console.log(this.data);
          })
         }
    ...
    //    console.log(event);   // this select return only one page rows
     ...
     public onEdit(event) {
      ...
      if(event.data.IsPosted != '1'){
        this.router.navigateByUrl('pages/purchase/purchasereturn/' + event.data.InvoiceID);
       }else{
    ...
    onCustom(event){
      ...
      this.http.getData('qrypinvoicedetails?filter=invoiceid='+event.data.InvoiceID).then(res=>{
        this.detaildata = res;
      })
    }
