  ...
  ngOnInit() {
    ...
    this.form.columns.forEach((col) => {
      if (col.hasOwnProperty("listTable") && col.listTable != "") {
        this.http.getData(col.listTable + "?flds=" + col.valueFld + "," + col.displayFld + "&orderby=" + col.displayFld).then((res: any) => {
          col.listData = res;
        });
      }
    });
  ...
  public SaveData() : void {
    ...
    if (!event.cancel) {
      ...
      if (this.files) {
        ...
        filedata.append("upload_folder", 'products');
  ...
  public onFiltering =  (e: FilteringEventArgs, data:any) => {
    ...
    if (e.text === '') {
      ...
      let filtered = data.filter(f=>{
        return JSON.stringify(f).toLowerCase().indexOf(e.text.toLowerCase() ) >=0;
      })
