...
export class CashsaleComponent implements OnInit, AfterViewInit {
  selectedProduct: any;
  @ViewChild("fromSale") fromSale;
  @ViewChild("cmbProd") cmbProd;
  @ViewChild("qty") elQty;
  @ViewChild("store") store;
  @ViewChild("cmbCustomer") cmbCust;
  public nWhat = "1"
          ...
          .then((res1: any) => {
            ...
            if (res1.length > 0) {
              if (res1[0].IsPosted !== "1") {
                ...
                this.Sale.netamount = res1[0].DtCr;
  ...
  public AddOrder() {
    if (this.nWhat == "2") {
      this.data.add({
        ...
        qty: this.productdetails.KGs * -1,
        Pending: 0,
        KGs: 0,
        amount: this.productdetails.SPrice * (this.productdetails.KGs * -1),
      ...
      if (this.productdetails.KGs* 1 >this.tqty * 1){
        this.myToaster.Error("Not enough stock", "Error");
        return;
      }
      this.data.add({
        description: this.cmbProd.text,
  ...
  public SaveData() {
    ...
    this.data.getAll().then((res1: any) => {
      // this.prtSrvc.InvoiceData.Details = Ores1.slice();
      let i = 0;
      // this.PrintData(res1);
  ...
  public calculation() {
    ...
    this.data.getAll().then(d => {
      ...
      this.Sale.paidby = this.Sale.netamount;
    });
  }
  ...
  FillCust() {
    this.http.getData("customers?filter=AcctTypeID=1").then(res => {
