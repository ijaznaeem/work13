...
class Apis extends REST_Controller
{
    ...
    public function uploadfile_post()
    {
        ...
        // Get folder and filename from POST
        ...
        if ($base64) {
            ...
            if (strpos($base64, ',') !== false) {
                ...
                $extmime = explode(',', $base64, 2)[0];
            }

            $decoded = base64_decode($base64);
            if ($decoded === false) {
                $this->response(['status' => 'error', 'msg' => 'Invalid base64 data'], REST_Controller::HTTP_BAD_REQUEST);
                return;
            }
            ...
            print_r($this->detectMimeType($base64));

            // Determine file extension from detected mime or filename or fallback
            $ext = isset($mime_map[$mime]) ? $mime_map[$mime] : 'bin';
            if ($filename) {
                $ext_from_name = pathinfo($filename, PATHINFO_EXTENSION);
                if ($ext_from_name) {
                    $ext = $ext_from_name;
                }
                $base_name = preg_replace('/[^a-zA-Z0-9_\-]/', '', pathinfo($filename, PATHINFO_FILENAME));
                $save_name = $base_name ? $base_name . ($ext ? '.' . $ext : '') : uniqid() . '.' . $ext;
            } else {
            ...
            if (file_put_contents($full_path, $decoded) !== false) {
                ...
                $this->response(['status' => 'error', 'msg' => 'Failed to save file'], REST_Controller::HTTP_BAD_REQUEST);
            }
        } else {
            // Handle file upload
            ...
            // If filename provided, sanitize and use it, else use encrypted name
            if ($filename) {
                $original_name = $_FILES[$file_element_name]['name'];
                $ext           = pathinfo($original_name, PATHINFO_EXTENSION);
                $base_name     = preg_replace('/[^a-zA-Z0-9_\-]/', '', pathinfo($filename, PATHINFO_FILENAME));
                if ($base_name) {
                    $_FILES[$file_element_name]['name'] = $base_name . ($ext ? '.' . $ext : '');
    ...
    public function detectMimeType($base64_data)
    {
        ...
        $temp_file = FCPATH . 'temp_upload_' . uniqid() . '.tmp';
        file_put_contents($temp_file, $binary_data);

        // Load file helper if needed
        $this->load->helper('file');

        // Detect MIME using CodeIgniter's helper (fallback to finfo if needed)
        $mime = get_mime_by_extension($temp_file);
    ...
    public function changepwd_post()
    {
        ...
        $retype_pwd = $this->post('retype_password');
        $user_id    = $this->post('user_id');

        if (! $old_pwd || ! $new_pwd || ! $retype_pwd || ! $user_id) {
            $this->response(['result' => 'Error', 'message' => 'Missing required fields'], REST_Controller::HTTP_BAD_REQUEST);
            return;
        }

        if ($new_pwd !== $retype_pwd) {
            $this->response(['result' => 'Error', 'message' => 'New password and re-typed password do not match'], REST_Controller::HTTP_BAD_REQUEST);
            return;
        }

        // Fetch user
        $user = $this->db->where('userid', $user_id)->get('users')->row_array();
        if (! $user) {
            $this->response(['result' => 'Error', 'message' => 'User not found'], REST_Controller::HTTP_NOT_FOUND);
            return;
        }

        // Check old password (assuming hashed with password_hash)
        if (($old_pwd!=$user['password'])) {
            $this->response(['result' => 'Error', 'message' => 'Old password is incorrect'], REST_Controller::HTTP_BAD_REQUEST);
            return;
        }
        ...
        // Update password
        $this->db->where('userid', $user_id)->update('users', ['password' => $new_pwd]);
        $this->response(['result' => 'Success', 'message' => 'Password changed successfully'], REST_Controller::HTTP_OK);
    ...
    public function resetpwd_post()
    {
      ...
      if (!$user_id || !$new_pwd || !$retype_pwd) {
        $this->response(['result' => 'Error', 'message' => 'Missing required fields'], REST_Controller::HTTP_BAD_REQUEST);
        return;
      }

      if ($new_pwd !== $retype_pwd) {
        $this->response(['result' => 'Error', 'message' => 'Passwords do not match'], REST_Controller::HTTP_BAD_REQUEST);
        return;
      }
      ...
      $this->db->where('userid', $user_id)->update('users', ['password' => $new_pwd]);
      $this->response(['result' => 'Success', 'message' => 'Password reset successfully'], REST_Controller::HTTP_OK);
    }
}
