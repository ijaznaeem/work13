...
class Apis extends REST_Controller
{
    ...
    public function cashreport_get()
    {
        ...
        $acct   = $this->db->query(
            ...
            " Select Date, 'Purchase' as Type, InvoiceID, CustomerName, Address,  AmountPaid as Debit, 0 as Credit from qrypinvoices where  AmountPaid > 0 AND $filter  and BusinessID = $bid UNION ALL " .
            " Select Date, 'Purchase' as Type, InvoiceID, CustomerName, 'Transportation',  0 as Debit, PackingCharges as Credit from qrypinvoices where  PackingCharges > 0 AND $filter  and BusinessID = $bid UNION ALL " .
            " Select Date, 'Purchase' as Type, InvoiceID, CustomerName, 'Unloading Charges',  0 as Debit, Unloading as Credit from qrypinvoices where  Unloading > 0 AND $filter  and BusinessID = $bid "
        )->result_array();
    ...
    public function getbno_get($type, $date)
    {
        ...
        $this->response(array('billno' => $maxInvoiceID), REST_Controller::HTTP_OK);
    }
...
public function getstock_get()
    {
        ...
        $result = $this->dbquery("SELECT distinct stock.ProductID, stock.ProductName, PCode FROM qrystock stock where BusinessID =  $bid");
...
// echo $post_data['message']; exit(0);
        ...
        $parameters = array("api_key" => "923424256584-0242e686-ae95-4ed9-b754-f3ae46f5e964",
    ...
    public function deletecustomeracct_post()
    {
        $this->load->database();
        ...
        try {
            $this->db->query("Call RecalculateCustomerBalance($detailid,$customerid)");
            $this->response(array('status' => true, 'message' => 'Record deleted'), REST_Controller::HTTP_OK);
        } catch (Exception $e) {
            $this->response(array('status' => false, 'message' => 'Error: ' . $e->getMessage()), REST_Controller::HTTP_INTERNAL_SERVER_ERROR);
        }
    }
