...
class Tasks extends REST_Controller
{
    ...
    public function purchase_post($id = null)
    {
        ...
        $pinvoice['LbrRate']        = $post_data['LbrRate'];
        ...
        $pinvoice['OnlineCash'] = $post_data['OnlineCash'];
        $pinvoice['BankID']     = $post_data['BankID'];
        $pinvoice['CashAmount'] = $post_data['CashAmount'];
        $pinvoice['Unloading']  = $post_data['Unloading'];
        ...
        foreach ($details as $value) {
            ...
            $pdetails['WPrice']     = $value['WPrice'] ?? 0;
            $pdetails['Carriage']   = $value['Carriage'];
    ...
    public function transfer_post($id = null)
    {
        ...
        $transfer['Description']     = $post_data['Description'];
        $transfer['LabourAmount']    = $post_data['LabourAmount'];
        $transfer['DeliveryCharges'] = $post_data['DeliveryCharges'];
    ...
    public function sale_post($id = null)
    {
        ...
        $invoice['LoadingCharges']  = $post_data['LoadingCharges'];
        ...
        $invoice['CashAmount']      = $post_data['CashAmount'];
        $invoice['OnlineCash']      = $post_data['OnlineCash'];
    ...
    private function PostVouchers($id = 0, $bid = 0)
    {
        ...
        foreach ($InvoiceRes as $InvoiceValue) {
            ...
            if ($InvoiceValue['BankCash'] > 0) {
                if ($InvoiceValue['RefType'] == 1) {
                    $data['Debit']  = $InvoiceValue['BankCash'];
                    $data['Credit'] = 0;
                } else {
                    $data['Credit'] = $InvoiceValue['BankCash'];
                    $data['Debit']  = 0;

                }
                $data['CustomerID']  = $InvoiceValue['BankAccountID'];
                $data['Date']        = $InvoiceValue['Date'];
                $data['Description'] = $InvoiceValue['Description'];
                $data['RefID']       = $InvoiceValue['VoucherID'];
                $data['RefType']     = 2;
    ...
    public function CloseAccount_post($bid, $bCloseAcct = 1)
    {
        $post_data = $this->post();
        try {
        ...
        if (count($InvoiceRes) > 0) {
            foreach ($InvoiceRes as $InvoiceValue) {
                ...
                if ($InvoiceValue['DtCr'] == 'CR') { // sale
                    ...
                    if ($InvoiceValue['OnlineCash'] > 0) {
                        $data['CustomerID']  = $InvoiceValue['BankID'];
                        $data['Date']        = $InvoiceValue['Date'];
                        $data['Debit']       = $InvoiceValue['OnlineCash'];
                        $data['Credit']      = 0;
                        $data['Description'] = 'Cash Recvd Bill No ' . $InvoiceValue['InvoiceID'];
                        $data['RefID']       = $InvoiceValue['InvoiceID'];
                        $data['RefType']     = 1;
                    ...
                    if ($InvoiceValue['DeliveryCharges'] > 0 && $InvoiceValue['TransporterID'] > 0) {
                        $data['CustomerID']  = $InvoiceValue['TransporterID'];
                        $data['Date']        = $InvoiceValue['Date'];
                        $data['Credit']      = $InvoiceValue['DeliveryCharges'];
                        $data['Debit']       = 0;
                        $data['Description'] = 'Delivery Charges Bill No ' . $InvoiceValue['InvoiceID'];
                        $data['RefID']       = $InvoiceValue['InvoiceID'];
                        $data['RefType']     = 1;
                        $data['BusinessID']  = $InvoiceValue['BusinessID'];
                        $this->AddToAccount($data);
    ...
    private function PostPurchases($id = 0, $bid = 0)
    {
        ...
        if (count($PInvoiceRes) > 0) {
            foreach ($PInvoiceRes as $PInvoiceValue) {
                $Carrriage = $PInvoiceValue['FrieghtCharges'];
                if ($PInvoiceValue['DtCr'] == 'CR') {
                    $data['CustomerID']  = $PInvoiceValue['CustomerID'];
                    $data['Date']        = $PInvoiceValue['Date'];
                    $data['Credit']      = $PInvoiceValue['NetAmount']; //$PInvoiceValue['FrieghtCharges'];
                    ...
                    $data['Debit']      = 0;
                    $this->AddToAccount($data);
                    if ($PInvoiceValue['CashAmount'] + $PInvoiceValue['OnlineCash'] > 0) {
                        $data['CustomerID']  = $PInvoiceValue['CustomerID'];
                        $data['Date']        = $PInvoiceValue['Date'];
                        $data['Debit']       = $PInvoiceValue['CashAmount'] + $PInvoiceValue['OnlineCash']; //$PInvoiceValue['FrieghtCharges'];
                        $data['Description'] = 'Purchase No ' . $PInvoiceValue['InvoiceID'];
                        $data['RefID']       = $PInvoiceValue['InvoiceID'];
                        $data['RefType']     = 2;
                        $data['Notes']       = $PInvoiceValue['Notes'];
                    ...
                    $Carrriage = $Carrriage / $this->FindQtySum($PInvoiceDetailsRes);
                    ...
                    foreach ($PInvoiceDetailsRes as $PInvoiceDetailsvalue) {
                        $this->UpdateStock(
                            ...
                            ($PInvoiceDetailsvalue['UnitID'] == 1 ? $PInvoiceDetailsvalue['WPrice'] : $PInvoiceDetailsvalue['PPrice']) + $Carrriage,
                            0,
                    ...
                    if ($PInvoiceValue['CashAmount'] + $PInvoiceValue['OnlineCash'] > 0) {
                        ...
                        $data['RefType']     = 2;
                        $data['BusinessID']  = 0;
                        $data['Credit']      = $PInvoiceValue['CashAmount'] + $PInvoiceValue['OnlineCash'];
                    }
                    $this->AddToAccount($data);
    ...
    private function FindQtySum($array)
    {
        $totalQty = 0;
        foreach ($array as $item) {
            $totalQty += $item['Qty'] * $item['Packing'] + $item['KGs'];
        }
        return $totalQty;
    }
