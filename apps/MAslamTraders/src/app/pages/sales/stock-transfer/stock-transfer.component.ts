...
import { StockTransfer, TraferDetails } from '../sale-invoice/sale.model';
...
export class StockTransferComponent implements OnInit, OnChanges {
  ...
  ProductSelected(product) {
    ...
    if (product) {
      ...
      this.transferdetails.UnitValue = product.UnitValue;
      this.selectedProduct.Weight = product.Weight;
    }
  }
  getTime(d: Date) {
  ...
  public SaveData(print = 0) {
    ...
    if (
      (this.stocktransfer.DeliveryCharges > 0 &&
        this.stocktransfer.TransporterID === '') ||
      this.stocktransfer.TransporterID === null
    ) {
      this.myToaster.Error('Please select Transporter', 'Error', 2);
      return;
    }
  ...
  public calculation() {
    this.stocktransfer.LabourAmount = 0;
    ...
    this.data.getAll().then((d) => {
      ...
      for (i = 0; i < d.length; i++) {
        this.stocktransfer.LabourAmount += 1 * d[i].Labour;
      }
    });
  }
  ...
  NavigatorClicked(e) {
    ...
    switch (Number(e.Button)) {
      ...
      case Buttons.Last:
        this.http
          .getData('stocktransfer?flds=Max(TransferID) as Bno')
          .then((r: any) => {
            billNo = r[0].Bno;
            console.log(r);
