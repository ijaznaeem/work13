...
export class DayBookComponent implements OnInit {
  ...
  FilterData() {
    ...
    if (this.nWhat === '1') {
      ...
      table = 'qryinvoices?orderby=InvoiceID ';
    ...
    this.http.getData(table + '&filter=' + filter).then((r: any) => {
      r.map((x) => {
        ...
        if (x.DtCr && x.DtCr === 'DT') {
          x.Amount = x.Amount * -1;
          x.Discount = x.Discount * -1;
          x.NetAmount = x.NetAmount * -1;
          x.RowClass = 'text-bg-light';
        }
      });
      this.data = r;
    });
      ...
      if (e.data.IsPosted === '0') {
        if (this.nWhat === '1') {
          if (e.data.Type == '1') {
            ...
            this.router.navigateByUrl('/sale/retail/' + e.data.InvoiceID);
          }
        } else if (this.nWhat === '2') {
  ...
  CloseAccounts() {
    swal({
      ...
      if (close) {
        this.http
          .postTask(`CloseAccount/${this.http.getBusinessID()}/0`, {
