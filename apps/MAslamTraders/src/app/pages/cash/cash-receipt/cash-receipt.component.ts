...
export class CashReceiptComponent implements OnInit {
  ...
  SaveData() {
    ...
    if (this.Voucher.BankCash > 0 && this.Voucher.BankAccountID === '') {
      this.alert.Error('Please select Bank Account', 'Error', 1);
      return;
    }
    ...
    this.http
      ...
      .then((r:any) => {
        ...
        if (this.EditID != '') {
          ...
          this.router.navigateByUrl('/cash/cashreceipt/' + r.id);
        }
      })
      .catch((err) => {
