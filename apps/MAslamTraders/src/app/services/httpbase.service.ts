...
export class HttpBase {
  ...
  getProducts(CatID = 0) {
    ...
    return this.getData(
      'qryproducts?flds=ProductID,PCode,ProductName,SPrice,BaseRate,PPrice,Weight,Packing,UnitID,Value as UnitValue&' +
        filter +
        '&orderby=ProductName'
    );
  }
  getStock(StoreID = '') {
    ...
    return this.getData(
      'qrystock?flds=ProductID,PCode,ProductName,Stock,SPrice,PPrice,Packing,Weight,UnitID,UnitValue as UnitValue&' +
        filter +
        '&orderby=ProductName'
    );
  }
  ...
  openFormWithEvents(form: any, formData: any = {}) {
    ...
    this.bsModalRef = this.modalService.show(ModalContainerComponent, initialState);
...
GetPrintData(){
    JSON.parse (localStorage.getItem('PRINT_DATA') || '{}');
}
