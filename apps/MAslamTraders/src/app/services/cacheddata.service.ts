...
public Stock$ = this._stockData$.pipe(
  ...
  //------ users list
  private _usersData$ = new BehaviorSubject<void>(undefined);
  public apiUsers$ = this.http.get<any[]>(this.api + 'users?bid=' + this.http2.getBusinessID());

  public Users$ = this._usersData$.pipe(
    switchMap(() => this.apiUsers$),
    shareReplay(1)
  );
  //------ companies list
