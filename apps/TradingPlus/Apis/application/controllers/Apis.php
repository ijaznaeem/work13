...
class Apis extends REST_Controller
{
    ...
    public function stockhistory_post()
    { // Get required parameters from POST
      ...
      if (! $this->checkToken()) {
        ...
        $toDate    = $this->post('ToDate');
        $storeID   = $this->post('StoreID');
        $productID = $this->post('ProductID'); // Add ProductID
        $businessID = $this->post('BusinessID'); // Add BusinessID

        if (empty($businessID) || empty($fromDate) || empty($toDate) || empty($storeID) || empty($productID)) {
            $this->response([
                'result'  => 'Error',
        ...
        // Load the Stock_mamangement library
        $this->load->library('stock_management');

        // Get stock transaction history, include productID if provided

        $result = $this->stock_management->get_transaction_history($productID, $storeID, $businessID, $fromDate, $toDate);
