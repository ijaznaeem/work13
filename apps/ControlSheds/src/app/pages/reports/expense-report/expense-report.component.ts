...
export class ExpenseReportComponent implements OnInit {
  ...
  FilterData() {
    ...
    this.http.getData('qryexpense?filter=' + filter).then((r: any) => {
      this.data = r;
    });
  }
  Clicked(e) {
    if (e.action === 'delete') {
      ...
      if (e.data.FlockID === this.http.GetFlockID()) {
        swal({
          ...
          if (willDelete) {
            this.http
              .getData('delete/expend/' + e.data.ExpendID)
              .then((r) => {
                this.FilterData();
        ...
        swal('Oops!', 'Can not delete previouse flock data', 'error');
      }
    }
  }
}
