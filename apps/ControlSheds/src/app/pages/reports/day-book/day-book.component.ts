...
import { StockConsumedComponent } from '../../tasks/stock-consumed/stock-consumed.component';
...
export class DayBookComponent implements OnInit {
  ...
  FilterData() {
    ...
    if (this.nWhat === '1') {
      ...
      table = 'qrystockconsumed?orderby=InvoiceID DESC';
  ...
  Clicked(e) {
    ...
    if (e.action === 'delete') {
      ...
      if (e.data.IsPosted === '0') {
        if (this.nWhat === '1') {
          this.bsModal = this.modal.openModal(StockConsumedComponent, {
            EditID: e.data.InvoiceID,
            Type: e.data.DtCr
          });
          ...
          this.bsModal = this.modal.openModal(PurchaseInvoiceComponent, {
            EditID: e.data.InvoiceID,
            Type: e.data.DtCr
          });
          this.bsModal.onHide.subscribe(() => {
          ...
          console.log(e.data);
          if (e.data.Credit > 0) {
            this.bsModal = this.modal.openModal(CashReceiptComponent, {
              EditID: e.data.VoucherID,
            });
            this.bsModal.onHide.subscribe(() => {
              this.FilterData();
            });
          } else {
            this.bsModal = this.modal.openModal(CashPaymentComponent, {
              EditID: e.data.VoucherID,
            });
            this.bsModal.onHide.subscribe(() => {
              this.FilterData();
      ...
      if (e.data.IsPosted === '0') {
        if (this.nWhat === '1') {
          url = 'poststockused/' + e.data.InvoiceID;
        } else if (this.nWhat === '2') {
