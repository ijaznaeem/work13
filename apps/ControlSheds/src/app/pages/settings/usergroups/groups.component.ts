...
export class GroupsComponent implements OnInit {
  ...
  public list = {
    pk: 'GroupID',
    Columns: [
      { fldName: 'GroupID', label: 'ID' },
      { fldName: 'GroupName', label: 'Group Name' },
    ],
    Actions: [
      {
        action: 'edit',
  ...
  LoadData() {
    this.http.getData('usergroups?nobid=1').then((data) => {
      this.data = data;
      this.GroupData = data;
    });
  }
  ...
  public SaveData1() {
    ...
    this.http
      ...
      .then(
        (res) => {
          this.http
            .postTask('grouprights', { GroupID: this.rightgroupid, data: data })
            .then(
              (res) => {
                this.cancel();
