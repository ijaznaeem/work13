...
class Tasks extends REST_Controller
{
  ...
  public function stockused_post($id = null)
  {
    $post_data = $this->post();
    ...
    $invoice['FlockID'] = $post_data['FlockID'];
    $invoice['Remarks'] = $post_data['Remarks'];
    $invoice['Day'] = $post_data['Day'];



    $this->db->trans_begin();
    if ($id == null) {
      $this->db->insert('stockused', $invoice);
      $invID = $this->db->insert_id();
      $details = $post_data['details'];
    } else {
      $this->db->where('InvoiceID', $id);
      $this->db->update('stockused', $invoice);
      $invID = $id;
      $details = $post_data['details'];
      $this->db->query("DELETE FROM `stockuseddetails` WHERE `InvoiceID`=" . $id);
    }
    foreach ($details as $value) {
      $dData = array(
        'InvoiceID' => $invID,
      ...
      $this->db->insert('stockuseddetails', $dData);
    }
  ...
  public function chickensale_post($id = 0)
  {
    $data = $this->post();

    $this->db->insert('chickensale', $data);
    $id = $this->db->insert_id();
    $data2['CustomerID'] = $data['CustomerID'];
    $data2['Date'] = $data['Date'];
    $data2['Debit'] = $data['Amount'];
    $data2['Credit'] = 0;
    $data2['Description'] = 'Chicken Sale';
    $data2['RefID'] = $id;
    $data2['RefType'] = 3;
    $data2['FlockID'] = $data['FlockID'];
    $data2['BusinessID'] = $data['BusinessID'];
  
    $this->AddToAccount($data2);

    if ($data['Received'] > 0) {
      $data2['CustomerID'] = $data['CustomerID'];
      $data2['Date'] = $data['Date'];
      $data2['Credit'] = $data['Received'];
      $data2['Debit'] = 0;
      $data2['Description'] = 'Chicken Sale Cash Receipt';
      $data2['RefID'] = $id;
      $data2['RefType'] = 3;
      $data2['FlockID'] = $data['FlockID'];
      $data2['BusinessID'] = $data['BusinessID'];
  ...
  public function poststockused_post($InvoiceID)
  {
    $this->PostStockUsed($InvoiceID);
    $this->response(array('msg' => 'Invoice Post'), REST_Controller::HTTP_OK);
  }
  ...
  private function PostStockUsed($id = 0, $bid = 0)
  {
    if ($id > 0) {
      $this->db->where('InvoiceID', $id);
    } else {
    ...
    $InvoiceRes = $this->db->get('qrystockconsumed')->result_array();
    $this->db->trans_begin();
    if (count($InvoiceRes) > 0) {
      foreach ($InvoiceRes as $InvoiceValue) {
        $this->db->where('InvoiceID', $InvoiceValue['InvoiceID']);
        $InvoiceDetailsRes = $this->db->get('qrystockuseddetails')->result_array();

        if ($InvoiceValue['Type'] == 'CR') { // sale
        ...
        $posted['IsPosted'] = '1';
        $this->db->where('InvoiceID', $InvoiceValue['InvoiceID']);
        $this->db->update('stockused', $posted);
      }
    }
    $this->db->trans_commit();
  }
      ...
      foreach ($PInvoiceRes as $PInvoiceValue) {
        if ($PInvoiceValue['DtCr'] == 'CR') {
          ...
          foreach ($PInvoiceDetailsRes as $PInvoiceDetailsvalue) {
            ...
            $data['Credit'] = $PInvoiceDetailsvalue['Amount'];
            ...
            $data['FlockID'] = $PInvoiceValue['FlockID'];
            $data['Debit'] = 0;
            $this->AddToAccount($data);
            $this->UpdateStock(
              1,
          ...
          foreach ($PInvoiceDetailsRes as $PInvoiceDetailsvalue) {
            ...
            $data['CustomerID'] = $PInvoiceValue['CustomerID'];
            $data['Date'] = $PInvoiceValue['Date'];
            $data['Debit'] = $PInvoiceDetailsvalue['Amount'];
            $data['Qty'] = $PInvoiceDetailsvalue['Qty'];
            $data['Rate'] = $PInvoiceDetailsvalue['PPrice'];
            $data['Description'] = $PInvoiceDetailsvalue['ProductName'];
            $data['RefID'] = $PInvoiceValue['InvoiceID'];
