...
class Auth extends REST_Controller
{
  ...
  public function login_post()
  {
    ...
    if (count($User) > 0) {
      ...
      $res = $this->db->query("select * from closing where BusinessID = 
        $BusinessID order by ClosingID DESC limit 1 ")->result_array();
        ...
        if ($res[0]['Status'] == 0) {
          if (strtotime($res[0]['Date']) == strtotime($date)) {
            ...
            $this->set_response($output, REST_Controller::HTTP_OK);
              ...
              $output = array('msg' => 'Please login on date: ' . 
                date("M j, Y", strtotime($res[0]['Date'])));
          ...
          if (strtotime($date) > strtotime($res[0]['Date'])) {
            ...
            $res = $this->db->query("select * from closing where ClosingID = " . 
              $this->db->insert_id())
              ->result_array();
  ...
  {
    $flockid = 0;
    $flock = $this->db->query("select FlockID   from flocks where Status = 1 and  BusinessID = " . $User['BusinessID'])->result_array();
    if (count($flock) == 0) {
      $flockid = '0';
    } else {
