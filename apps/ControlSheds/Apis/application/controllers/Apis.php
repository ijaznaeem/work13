...
class Apis extends REST_Controller
{
    ...
    public function index_get($table = "", $id = "", $rel_table = null)
    {
        ...
        if ($this->get('bid') != "") {
            ...
            throw new Exception("no businessid given", 1);
    ...
    public function profitreport_get($flockid)
    {
        if (!$this->checkToken()) {
            $this->response(
                array(
        ...
        $this->load->database();
        $bid = $this->get('bid');
        $filter = "BusinessID = $bid and FlockID = $flockid";
        $query = $this->db->query(
            "'Purchases' as Type, CustomerNamae as Description, NetAmount  * GetCtCr(DtCr) as Amount, -1 as id FROM qrypinvoice WHERE $filter  GROUP BY  CustomerName " .
            " Union All " .
            "'Expenses' as Type, HeadName as Description,  Amount, -1 as id FROM qryexpnese WHERE $filter  GROUP BY  HeadName" .
            " Union All " .
            "'Sale' as Type, CustomerNamae as Description, NetAmount  * GetCtCr(DtCr) as Amount FROM qrychickensale WHERE $filter  GROUP BY  CustomerName "
    ...
    public function cashreport_get()
    {
        ...
        $acct = $this->db->query(
            ...
            " Select Date, 'Exepnse' as Type, Description, HeadName, Amount as Debit,0  as Credit from qryexpense where   $filter and BusinessID = $bid "
        );
    ...
    function deleteall_get($table = "", $atr = "", $id = 0, $reltable = "")
    {
        $this->load->database();
        if ($this->db->table_exists($table)) {
            $this->db->trans_start();
    ...
    public function startflock_post()
    {
        $this->output->set_header('Last-Modified: ' . gmdate("D, d M Y H:i:s") . ' GMT');
        $this->output->set_header('Cache-Control: no-store, no-cache, must-revalidate, post-check=0, pre-check=0');
        $this->output->set_header('Pragma: no-cache');
        ...
        $id = $this->StartNewFlock($bid);

        $this->response(array('id' => $id), REST_Controller::HTTP_OK);
    }

    public function StartNewFlock($bid)
    {
        $this->db->trans_begin();
        $acct = $this->db->query(" select max(flockid) as id from flocks where BusinessID = $bid")->result_array();
        if ($acct[0]['id'] != null) {
            $id = $acct[0]['id'];
        } else {
            $id = $bid * 1000000000;
        ...
        $id++;
        $flk = $this->db->query("select Max(FlockID) as id from flocks where BusinessID =$bid")
            ->result_array()[0]['id'];

        $this->db->query("update flocks set Status = 0 where FlockID = " . $flk);
    ...
    public function getbdata_get($bid)
    {

        $flockid = 0;
        $flock = $this->db->query("select FlockID   from flocks where Status = 1 and  BusinessID = " . $bid . " Order by FlockID DESC")->result_array();
        if (count($flock) == 0) {
            $flockid = '0';
        } else {
            $flockid = $flock[0]['FlockID'];
        }

        $closing = $this->db->query("select *   from closing WHERE BusinessID =  $bid  order by ClosingID DESC Limit 1")->result_array();
        if (count($closing) == 0) {

            $this->response(array('result' => 'Error', 'message' => 'Account is not opened'), REST_Controller::HTTP_INTERNAL_SERVER_ERROR);
            return;
        } else if ($closing[0]['Status'] == 1) {

            $this->response(array('result' => 'Error', 'message' => 'Account closed on ' . $closing[0]['Date']), REST_Controller::HTTP_INTERNAL_SERVER_ERROR);
            return;
        }

        $bdata = $this->db->query("select *  from business where BusinessID = " . $bid)->result_array()[0];
        $output['flockid'] = $flockid;
        $output['date'] = $closing[0]['Date'];
        $output['bdata'] = $bdata;
        $output['closingid'] = $closing[0]['ClosingID'];
