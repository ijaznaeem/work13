...
class Auth extends REST_Controller
{
  ...
  public function login_post()
  {
    ...
    if (count($User) > 0) {
      ...
      $b = $this->db->query("select * from business where BusinessID = $BusinessID")->result_array()[0];
      if (strtotime($b['ExpiryDate']) < strtotime(date('Y-m-d'))) {
        $this->response(array('status' => 'false',
          'msg' => 'You account has been expired ' . $b['ExpiryDate']),
      ...
      if (count($res) > 0) {
        ...
        if ($res[0]['Status'] == 0) {
          if (strtotime($res[0]['Date']) == strtotime($date)) {
            $output = $this->CreateOutput($User, $res[0]);
          ...
          $close_date = date('Y-m-d', strtotime($res[0]['Date'] . ' +1 day'));
          $date = date('Y-m-d', strtotime($date));
          //   echo( 'close_date: ' . $close_date);
          // echo( 'date: ' . $date);
          ...
          // exit();
            ...
            $OpnAmnt = $res[0]['ClosingAmount'];
              ...
              $output['msg'] = 'Please login on date: ' . date("M j, Y", strtotime($close_date));
              $this->set_response($output, REST_Controller::HTTP_BAD_REQUEST);
            }
          } else {
  ...
  public function CreateOutPut($User, $closing)
  {
    ...
    $output['BusinessData'] = $this->db->query("select * from business where BusinessID = " . $User['BusinessID'])->result_array()[0];
