...
export class DayBookComponent implements OnInit {
  ...
  ngOnInit() {
    this.otherssetting = JSON.parse(JSON.stringify(VoucherSetting));
    this.otherssetting.Actions = [];
  ...
  FilterData() {
    ...
    this.http
      .getData("closing?filter=Date='" + JSON2Date(this.Filter.FromDate) + "'")
      .then((r: any) => {
        if (r.length > 0) {
        ...
        this.http
          .getData('qryvouchers?orderby=VoucherID' + '&filter=' + filter)
          .then((r: any) => {
            this.vdata = r;
            ...
            this.http
              ...
              filter += " AND (BusinessID <> CBID and CBID=" + this.http.getBusinessID() + ")";
            this.http
              .getData('qryvouchers', {orderby: 'VoucherID', filter: filter, bid:0})
              .then((r: any) => {
                this.vothers = r;
                if (this.vothers && this.Businesses) {
                  this.vothers.forEach((item: any) => {
                    const business = this.Businesses.find((b: any) => b.BusinessID === item.BusinessID);
                    if (business) {
