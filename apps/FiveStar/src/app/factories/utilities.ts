...
export function GetFilter(fltr: any) {
  ...
  props.forEach((p) => {
    if (fltr[p]) filter += ' and ' + p + " = '" + fltr[p] + "'";
  });
  return filter;
}
export function GetPhoneNos(data: any = [], fld) {
  let PhoneNos: any = [];
  console.log(data);

  for (let i = 0; i < data.length; i++) {
    PhoneNos.push(data[i][fld]);

  }
  return PhoneNos;
}

export function  divideAndTruncate(number, divisor) {
  var result = number / divisor; // Perform the division
  var decimalPart = result % 1; // Get the decimal part of the result

  if (decimalPart > 0) {
      result = Math.floor(result) + 1; // Increase the result by 1 if there is a decimal part
  } else {
      result = Math.floor(result); // Otherwise, simply truncate the result
  }
...
export function AddDays(date: Date, daysToAddOrSubtract: number): Date {
  const result = new Date(date);
  result.setDate(result.getDate() + daysToAddOrSubtract);
  return result;
}
