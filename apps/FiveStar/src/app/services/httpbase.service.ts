...
export class HttpBase {
  ...
  getProducts(CatID = 0) {
    ...
    return this.getData(
      'qryproducts?flds=ProductID,ProductName,SPrice,PPrice,Packing,Weight,Value as UnitValue&' +
        filter +
        '&orderby=ProductName'
    );
  }
  ...
  getBusinessData() {
    return JSON.parse(localStorage.getItem('currentUser') || '{}').BusinessData;
  }
  getClosingDate() {
  ...
  getCustByType(rtid = '') {
    ...
    return this.getData(
      'qrycustomers?flds=CustomerName,Address, PhoneNo1,MBalance, Balance,CustomerID&orderby=CustomerName' +
        ...
        html: `
          ...
          <div class="custom-control mt-2 custom-radio">
              <input
                ...
                value="4"
              />
              <label class="custom-control-label text-danger" for="Store4">
                <b>Store 1</b>
              </label>
            </div>
          </div>
  ...
  PrintOrder(InvoiceID: any) {

    window.open(
      '/#/print/printorder/' + InvoiceID,
      '_blank',
      'toolbar=1, scrollbars=1, resizable=1, width=' + 600 + ', height=' + 800
    );
  }
