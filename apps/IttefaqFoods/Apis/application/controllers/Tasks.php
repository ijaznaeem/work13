...
class Tasks extends REST_Controller
{
  ...
  private function PostVouchers($id = 0)
  {
    ...
    foreach ($InvoiceRes as $InvoiceValue) {
      ...
      $data['ProductID'] = $InvoiceValue['ProductID'];
  ...
  public function delete_post()
  {
    ...
    } elseif ($post_data['Table'] === 'Q') {
      $this->db->query('delete from quotations where InvoiceID=' . $post_data['ID']);
      $this->db->query('delete from quotationdetails where InvoiceID=' . $post_data['ID']);
    }
    $this->db->trans_commit();
    ...
    if (count($InvoiceRes) > 0) {
      foreach ($InvoiceRes as $InvoiceValue) {
        ...
        foreach ($InvoiceDetailsRes as $InvoiceDetailsvalue) {
          ...
          $data['RefID'] = $InvoiceDetailsvalue['InvoiceID'];
          $data['ProductID'] = $InvoiceDetailsvalue['ProductID'];
          $data['RefType'] = 1;

          $this->AddToAccount($data);

          if ($InvoiceDetailsvalue['StockAble']==1){
          $this->UpdateStock(
            2,
        ...
        if ($InvoiceValue['Discount'] > 0) {
          $data['CustomerID'] = $InvoiceValue['CustomerID'];
          $data['Date'] = $InvoiceValue['Date'];
          $data['Credit'] = $InvoiceValue['DtCr'] == 'CR' ? $InvoiceValue['Discount'] : 0;
          $data['Debit'] =   $InvoiceValue['DtCr'] == 'CR' ? 0 : $InvoiceValue['Discount'];
          $data['Description'] = $InvoiceValue['DtCr'] == 'CR' ? 'Discount ' : 'Discount Return ' . $InvoiceValue['InvoiceID'];
        ...
        if ($InvoiceValue['AmountRecvd'] > 0) {
          $data['CustomerID'] = $InvoiceValue['CustomerID'];
          $data['Date'] = $InvoiceValue['Date'];
          $data['Credit'] = $InvoiceValue['DtCr'] == 'CR' ? $InvoiceValue['AmountRecvd'] : 0;
          $data['Debit'] =   $InvoiceValue['DtCr'] == 'CR' ? 0 : $InvoiceValue['AmountRecvd'];
          $data['Description'] = $InvoiceValue['DtCr'] == 'CR' ? 'Cash Recvd Sale #' : 'Sale Return ' . $InvoiceValue['InvoiceID'];
          $data['RefID'] = $InvoiceValue['InvoiceID'];
          $data['RefType'] = 1;
          $data['BusinessID'] = $InvoiceValue['BusinessID'];
          $this->AddToAccount($data);
    ...
    if (count($PInvoiceRes) > 0) {
      ...
      foreach ($PInvoiceRes as $PInvoiceValue) {
        ...
        foreach ($PInvoiceDetailsRes as $PInvoiceDetailsvalue) {
          if ($PInvoiceDetailsvalue['StockAble']==1){
  ...
  public function updatewastage($rate){
    $this->db->query("update wastage set Rate = $rate, Credit = Wastage * $rate where rate = 0 and TrType = 1");
    $this->response(array('msg' => 'rates are updated'), REST_Controller::HTTP_OK);
  }
}
