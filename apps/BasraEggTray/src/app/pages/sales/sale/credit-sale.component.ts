...
export class CreditSaleComponent implements OnInit, OnChanges {
  ...
  ProductSelected(product) {
    ...
    if (product) {
      ...
      if (this.sale.CustomerID) {
        this.http
          .getData(
            'qrysalereport?flds=Date,SPrice,Qty,DiscRatio&orderby=InvoiceID Desc&limit=1&filter=CustomerID=' +
  ...
  getStock() {
    this.http.getData('qrystock?orderby=ProductName&filter=TypeID=2').then((res: any) => {
      this.Prods = res;
    });
  }
