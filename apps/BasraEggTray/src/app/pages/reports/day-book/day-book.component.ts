...
export class DayBookComponent implements OnInit {
    ...
    if (e.action === 'delete') {
      ...
      if (e.data.IsPosted === '0') {
        if (this.nWhat === '3') {
          ...
          table = { ID: e.data.ProductionID, Table: 'PR' };
        }
        swal({
      ...
      if (this.nWhat === '1') {
        ...
        window.open('/#/print/printpurchase/' + e.data.InvoiceID);
      } else if (this.nWhat === '3') {
        window.open('/#/print/print-voucher/' + e.data.VoucherID);
  ...
  CloseAccounts() {
    swal({
      text: 'All data will be posted, Continue ??',
      icon: 'warning',
      buttons: {
        cancel: true,
        confirm: true,
