...
export class VerticalMenuComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  LoadMenu() {
    ...
    if (this.http.getUserDept()== UserDepartments.grpCEO) {
      ...
      let menuFltred = ROUTES.map((element) => {
        return {
          ...element, submenu: element.submenu.filter((subElement) => {
            console.log(subElement.id);

            return (userMenu.filter(x => x.pageid == subElement.id)).length > 0
          })
        }
      }).filter(m => {
        return (((userMenu.filter(x => x.pageid == m.id)).length > 0) || m.submenu.length)
      })
      console.log(menuFltred)
      return menuFltred
    }
  }
  ngAfterViewInit() {
