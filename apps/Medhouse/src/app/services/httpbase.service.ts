...
import { API_URL, DirectorsGroup, UserDepartments } from '../config/constants';
...
export class HttpBase {
  ...
  getData(table, param: any = {}) {
    ...
    return new Promise((resolve, reject) => {
      this.http
        ...
        .subscribe({ next: (v) => resolve(v), error: (e) => reject(e) }
        );
    });
  }
  ...
  getStock(StoreID = '') {
    ...
    return this.getData(
      'qrystock?flds=ProductID,ProductName,SPrice,PPrice,Packing,UnitValue as UnitValue&' +
        filter +
        '&orderby=ProductName'
    );
  }
  ...
  getUserDept(): UserDepartments {
    return JSON.parse(localStorage.getItem('currentUser') || '{}').DeptID;
  }
  getUserMenu() {
  ...
  openFormModal(form: any, formData: any = {}, size = 'lg'): BsModalRef {
    const initialState: ModalOptions = {
      initialState: { form: form, formdata: formData },
      class: 'modal-' + size,
      backdrop: true,
  ...
  getCustList(rt = '') {
    ...
    return this.getData(
      'customers?flds=CustomerName,Address,Balance,CustomerID&orderby=CustomerName' +
        filter
    );
  }
  getAcctstList(type = '') {
