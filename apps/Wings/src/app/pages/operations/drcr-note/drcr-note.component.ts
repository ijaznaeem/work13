...
export class DrCrNoteComponent implements OnInit {
  ...
  FilterData() {
    ...
    this.http
      .getData('qrydrcr_note', { filter: filter, orderby: 'date' })
      .then((r: any) => {
        this.data = r;
      });
  }
  ...
  async AddVoucher(d: any) {
    ...
    let cnt: any = await this.http.getData('qrydrcr_note', {
      filter: 'note_id=' + d.ref_no,
    });
    if (cnt[0].status == 'Un-Paid') {
      d.debit = cnt[0].credit - cnt[0].total_debit;
      d.credit = cnt[0].debit - cnt[0].total_credit;
      const initialState: ModalOptions = {
        initialState: {
      ...
      this.myAlert.Error('Voucher already paid', 'Error');
    }
  }
}
