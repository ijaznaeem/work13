...
export class SaleOrdersComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  constructor(
    ...
    this.$airports = this.cachedData.airPorts$;
    this.$airLines = this.cachedData.Airlines$;
  ...
  public AddOrder() {
    ...
    if (this.sdetails.product_id =='' || this.sdetails.product_id == null ){
      this.myToaster.Error('Product is not selected', 'Error');
      return ;
    }
