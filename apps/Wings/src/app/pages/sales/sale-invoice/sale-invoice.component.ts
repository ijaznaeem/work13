...
import { ODetailsSettings } from '../sale-orders/orders.setting';
...
export class SaleInvoiceComponent implements OnInit, OnDestroy, AfterViewInit {
  @ViewChild('cmbProd') cmbProd;
  @Input() EditID = 0;
  public settings = ODetailsSettings;
  public airports: any = [];
  sale = new SaleOrderModel();
  sdetails: any = {};
  ...
  ngOnInit() {
    ...
    this.LoadCustomer(this.sale.customer_type);
  ...
  ngAfterViewInit() {
    if (+this.EditID > 0) {
      this.http.getData('invoices/' + this.EditID).then((data: any) => {
        this.sale.customer_type = data.customer_type + '';
        this.LoadCustomer(this.sale.customer_type);
        ...
        setTimeout(() => {
          ...
          this.calculation();
          this.FindBC(this.sale.charges_id);
        }, 1000);
        this.http
          .getData('qryinv_details?filter=invoice_id=' + this.EditID)
          .then((data: any) => {
  ...
  AddToDetails(ord: any) {
    ...
    const obj = {
      ...
      post_date: JSON2Date(ord.post_date),
  ...
  calculation() {
    ...
    this.data.getAll().then((d) => {
      ...
      for (i = 0; i < d.length; i++) {
        ...
        this.sale.vat += d[i].vat_value * 1;
        this.sale.markup += d[i].markup * 1;
      }
    });
  }
  ...
  public SaveData(p) {
    ...
    this.data.getAll().then((res1: any) => {
      ...
      this.sale.details = details;
      this.http.postTask('invoice' + InvoiceID, this.sale).then(
        (r: any) => {
          this.myToaster.Sucess('Invoice Saved', 'Save');
          if (p == 1) window.open('/print/invoice/' + r.id);
          this.EditID = 0;
          this.Cancel();
        },
        (err) => {
  ...
  Cancel() {
    this.data.empty();
    this.data.refresh();
    this.sale = new SaleOrderModel();
    this.sale.date = GetDateJSON();
    this.sdetails = new OrderDetailsModel();
  ...
  FindBC(v) {
    ...
    if (bc) {
      ...
      this.sale.charges_id = '1';
      this.sale.bank_charges = '0';
      this.bratio = 0;
    }
    console.log(bc);
  ...
  calcBC(v) {
    ...
    if (this.bratio > 0) {
      this.sale.bank_charges =
        ((this.sale.amount * 1 + this.sale.vat * 1 + this.sale.markup * 1) *
          v) /
        100;
    }
  }
