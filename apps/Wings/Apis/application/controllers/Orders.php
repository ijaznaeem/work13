...
class Orders extends REST_Controller
{
    ...
    public function invoice_post()
    {
        ...
        try {
            ...
            $qry = "insert into invoicedetails (InvoiceID, ProductID, StockID,  PPrice, SPrice, Qty, Pcs , Strength, Bonus, DiscRatio , SchemeRatio , GSTRatio , Remarks , RateDisc, BusinessID) " ;
            $qry .= " select $invID, ProductID, StockID,  PPrice, SPrice, Qty, Pcs , Strength, Bonus, DiscRatio , SchemeRatio , GSTRatio , Remarks , RateDisc, BusinessID from orders where ";
            $qry .= " Status =0 and Date = '" . $post_data['Date'] . "' and   CustomerID=" . $post_data['CustomerID'];

            $this->db->query($qry);
            $this->db->query("call procupdate_inv($invID)");
            $this->db->query("update orders set status = 1 where Date = '" . $post_data['Date'] . "' and   CustomerID=" . $post_data['CustomerID']);
    ...
    public function customers_get($date, $routeid=0)
    {
        ...
        $query = $this->db->query("select CustomerID, CustomerName from customers where customerid in " .
            " (Select CustomerID from orders where Status = 0 and Date = '$date') and (RouteID = $routeid or $routeid = 0)");
        if ($query) {
            $this->response($query->result_array(), REST_Controller::HTTP_OK);
        } else {
            $this->response(array('result' => 'Error', 'message' => $this->db->error()), REST_Controller::HTTP_BAD_REQUEST);
