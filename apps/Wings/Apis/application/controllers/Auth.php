...
class Auth extends REST_Controller
{
  ...
  public function login_post()
  {
    ...
    $User = $this->db->get('qryusers')->result_array();
    if (count($User)) {
      $User = $User[0];

      $this->db->where('branch_id', $User['branch_id']);
      $Branch = $this->db->get('branches')->result_array()[0];
      ...
      // $output['profileid'] = $User[0];
      $output['dept_id'] = $User['dept_id'];
      $output['branch_id'] = $User['branch_id'];
      $output['userid'] = $User['userid'];
      $output['grouprights'] = $this->getGroupRights($User['group_id']);
      $output['status'] = 'true';
      $output['user_data'] = $User;
      $output['branch_data'] = $Branch;
  ...
  private function getGroupRights($group_id)
  {

    $group = $this->db
      ->where('group_id', $group_id)
      ->get('usergrouprights')->result_array();
