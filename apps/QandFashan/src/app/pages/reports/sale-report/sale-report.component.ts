...
export class SaleReportComponent implements OnInit {
  ...
  public data: object[];
  public nWhat = '1';
  public settings:any = SaleSmryStngs;
  ...
  FilterData() {
    ...
    if (this.nWhat == '1') {
      const flds =
        'date,Count(*) as NoInv,  sum(Amount) as Amount, sum(NetDisc) as Discount, ' +
  ...
  Clicked(e) {
    ...
    if (e.action === 'print2') {
      this.http.getData('print/print-invoice/' + e.data.InvoiceID).then((d: any) => {
        d.Business = this.http.GetBData();
        console.log(d);
