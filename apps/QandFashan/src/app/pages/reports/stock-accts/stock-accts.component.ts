...
export class StockAcctsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.http
      .getData('qrystock?flds=ProductID,ProductName&orderby=ProductName')
      .then((r: any) => {
        this.Products = r;
      });
  ...
  FilterData() {
    ...
    this.http
      .getData(
        'qrystockaccts?flds=Date,Time,RefID, QtyIn, QtyOut, Balance, AccountName' +
          ' &filter=' +
          filter +
          '&orderby=AcctID'
      )
  ...
  getData(term: string | null = null): Observable<any> {
    return this.http.getApis(
      environment.INSTANCE_URL +
        'apis/qrystock?flds=ProductID, PCode, ProductName' +
