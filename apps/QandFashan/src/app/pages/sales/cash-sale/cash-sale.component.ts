...
export class CashSaleComponent implements OnInit {
  ...
  public SaveData() {
    if (this.sale.Amount > 0 && this.sale.ExtraDisc > this.sale.Amount) {
      this.myToaster.Error('Invalid Discount Amount', 'Error');
      return;
    }
    if (this.sale.AmountRecvd > 100000) {
      this.myToaster.Error('Invalid Cash Receieved Amount', 'Error');
      return;
    }
  ...
  PrintBill(r = {}) {
    ...
    if (this.bPrint) {
      ...
      this.sale['UserName'] = this.http.getUserData().FullName;
  ...
  public calculation() {
    ...
    for (i = 0; i < this.saledata.length; i++) {
      ...
      this.sale.Discount += this.saledata[i].Discount * 1;
      this.sale.Items += this.saledata[i].Qty * 1;
    }
    this.sale.NetAmount = RoundTo2(this.sale.NetAmount);
    this.sale.Amount = RoundTo2(this.sale.NetAmount + this.sale.Discount);
    this.sale.Rounding = RoundTo2(
      Math.round(this.sale.NetAmount / 10) * 10 - this.sale.NetAmount
    );
    this.sale.NetAmount += this.sale.Rounding;
    this.sale.Discount = RoundTo2(this.sale.Discount);
  }
  ...
  Find() {
    ...
    if (code.substring(0, 1) == '2') {
      qty = RoundTo2(Number(code.substring(code.length - 5, code.length - 1)) / 1000);
      code = code.substring(1, code.length - 6);
      console.log(qty, code);
    }
