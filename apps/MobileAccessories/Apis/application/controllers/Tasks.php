...
class Tasks extends REST_Controller
{
    ...
    public function invoice_post($id = null)
    {
        ...
        $invoice['OrderID']    = isset($post_data['OrderID']) ? $post_data['OrderID'] : 0;
        $invoice['IsPosted']   = isset($post_data['IsPosted']) ? $post_data['IsPosted'] : 0;
        ...
        try {
            ...
            // Update order status if invoice is created from order
            if (isset($post_data['OrderID']) && $post_data['OrderID'] > 0) {
                $this->db->where('OrderID', $post_data['OrderID']);
                $this->db->update('orders', ['IsPosted' => 1]);
            }
            ...
            $this->response(['error' => $e->getMessage()], REST_Controller::HTTP_BAD_REQUEST);
        }
    ...
    public function vouchers_post($id = 0)
    {
        ...
        try {
            $data = [
                ...
                'CustomerID'  => $posted_data['CustomerID'],
                'Description' => $posted_data['Description'],
                'Debit'       => $posted_data['Debit'],
                'Credit'      => $posted_data['Credit'],
                'IsPosted'    => $posted_data['IsPosted'],
            ];
            ...
            // Get the inserted voucher from qryvouchers and return it
            $this->db->where('VoucherID', $id);
            $voucher = $this->db->get('qryvouchers')->row_array();
        ...
        if (count($InvoiceRes) > 0) {
            foreach ($InvoiceRes as $InvoiceValue) {
                ...
                if ($InvoiceValue['DtCr'] == 'CR') {
                    $data['Credit'] = 0;
                    $data['Debit']  = $InvoiceValue['Amount'];
                } else {
                    $data['Debit']  = 0;
                    $data['Credit'] = $InvoiceValue['Amount'];
                }

                $data['Description'] = 'Sale Bill #' . $InvoiceValue['InvoiceID'];
    ...
    public function posttransfer_post($id = 0)
    {
        ...
        $this->db->trans_commit();
        $this->response(['msg' => 'stock transfer is posted'], REST_Controller::HTTP_OK);
