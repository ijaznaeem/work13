...
import { RoundTo, RoundTo2 } from '../factories/utilities';
...
export class PrintBillService {
  ...
  PrintPDFBill(data: any) {
    ...
    doc.setFontSize(10);
    y += 5;
    doc.text("Aqeel: 03403066006", midX, y, { align: 'center' });
    y += 5;
    doc.text("Nabeel: 03187835470", midX, y, { align: 'center' });
    y += 8;
    doc.text('Cash Memo', midX, y, { align: 'center' });
    y += 6;
    doc.setFont('Helvetica', '');
    doc.setFontSize(8);
    doc.text('Date:' + moment( data.Date).format('DD-MM-YY') + ' ' + data.Time, x, y, { align: 'left' });
    ...
    data.details.forEach((d) => {
      billBody.push([
        d.ProductName + (d.Packing != 1? ' (' + d.Packing.toString().replace('.00', '') + ')': ''),
        RoundTo(d.Qty, 1),
        d.SPrice,
    ...
    doc.text(
      RoundTo(data.Discount + data.ExtraDisc * 1, 2) + '',
      this.billW - x,
      y,
      {
        align: 'right',
    ...
    doc.text('Rounding: ', x + 30, y);
    doc.text(RoundTo(data.Rounding, 2) + '', this.billW - x, y, {
      align: 'right',
    });
    y += 6;
    doc.setFontSize(12);
    doc.text('Net Amount: ', x + 30, y);
    doc.text(RoundTo(data.NetAmount, 0) + '', this.billW - x, y, {
      align: 'right',
    });
    ...
    if (data.Type==2){
      ...
      doc.text(RoundTo((data.PrevBalance*1 + data.NetAmount* 1), 0) + '', this.billW - x, y, {
        align: 'right',
      });
    ...
    y += 6;
    doc.setFontSize(10);
    doc.text('Paid: ', x + 30, y);
    doc.text(RoundTo(data.AmountRecvd, 0) + '', this.billW - x, y, {
      align: 'right',
    });
    y += 6;
    doc.text('Change: ', x + 30, y);
    doc.text(RoundTo((data.AmountRecvd > data.NetAmount? data.AmountRecvd - data.NetAmount: 0), 0) + '', this.billW - x, y, {
      align: 'right',
    });
  }
    y += 6;

    doc.rect(29, y - 40, this.billW - 30, 36);

    doc.setFontSize(8);
    doc.text('Salesman: ' + data.SalesmanName, x, y + 4);
