...
import { AddLookupFld, AddFormButton } from '../../../../../../../libs/future-tech-lib/src/lib/components/crud-form/crud-form-helper'
...
export class AddRecoveryComponent implements OnInit {
  ...
  FilterData() {
    ...
    let filter = "Date = '" +   getYMDDate( new Date(this.http.getClosingDate())) + "'";


    filter += ' and RouteID = ' + this.Filter.RouteID;
    filter += ' and Balance > 0'
    this.http.getData('qryinvoices?flds=CustomerID,CustomerName,Address,City,Balance as Amount,\'0.0\' as Recovery,  Balance&filter=' + filter)
      .then((d: any) => {
        this.VouchersList = d;
      })
  }
