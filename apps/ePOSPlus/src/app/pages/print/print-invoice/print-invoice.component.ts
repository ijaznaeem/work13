...
import { FindTotal, formatNumber, RoundTo, RoundTo2 } from '../../../factories/utilities';
...
export class PrintInvoiceComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    this.route.paramMap.subscribe(params => {
      ...
      this.http.getData('qryinvoices?filter=Invoiceid=' + this.InvoiceID).then((inv: any) => {
        this.Invoice = inv[0];
        this.http.getData('warranties?filter=Type=' + this.Invoice.Type).then((w:any)=>{
          this.Invoice.Warranty = w[0].Warranty;
          this.ref.markForCheck();
          console.log(this.Invoice);

        })

        this.Invoice.BalanceInWords = (new ToWords()).convert(this.Invoice.Balance);
  ...
  async SaveAsPdf() {
    let docDefinition = {
      content: [
        ...
        {
          text:
            this.Invoice.DtCr == "CR" ? "SALE INVOICE" : "RETURN INVOICE",
          alignment: "center",
          fontSize: 20,
          bold: true,
        },
  ...
  buildTableBody() {
    ...
    this.Invoice.Detail.forEach((det, index) => {
      ...
      detBody.push({ text: formatNumber(this.RoundIt(det.SPrice, 0)), alignment: 'right' });
      detBody.push({ text: formatNumber(this.RoundIt(det.Amount, 0)), alignment: 'right' });
      detBody.push({ text: formatNumber(det.Discount), alignment: "right" });
      detBody.push({ text: formatNumber(this.RoundIt(det.NetAmount, 0)), alignment: 'right' });
