...
class Apis extends REST_Controller
{
  ...
  public function profitreport_get()
  {
    if (!$this->checkToken()) {
      $this->response(
        array(
    ...
    $income = $this->db->query("SELECT JobName as Description,  Sum(Amount) as Income, 0 as Expense FROM qryorders WHERE   " . $filter . " Group By JobName")->result_array();
    $expense = $this->db->query("SELECT HeadName, 0 as Income, Sum(Amount) as Expense FROM qryexpenses WHERE  " . $filter . " Group by HeadName")->result_array();
    ...
    $this->response($merged, REST_Controller::HTTP_OK);
  }
