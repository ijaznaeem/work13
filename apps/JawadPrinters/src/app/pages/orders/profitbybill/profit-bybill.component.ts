...
@Component({
  ...
  templateUrl: './profit-bybill.compenent.html',
...
export class ProfitByBillComponent implements OnInit {
  ...
  public Settings = {
    Columns: [
      ...
      { fldName: 'Expense', label: 'Expense' },
      { fldName: 'Profit', label: 'Profit' },
  ...
  FilterData(e) {
    ...
    let filter = `Date Between '${this.Filter.FromDate}' and '${this.Filter.ToDate}' And Status = 1`

    //filter = '1=1';
    this.http
      .getData('qryorders?filter=' + filter + '&orderby=Date')
      .then((d: any) => {
        this.data = [...d];
