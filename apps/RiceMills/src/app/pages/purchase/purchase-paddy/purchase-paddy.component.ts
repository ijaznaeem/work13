...
@Component({
  selector: 'app-purchase-paddy',
  templateUrl: './purchase-paddy.component.html',
  styleUrls: ['./purchase-paddy.component.scss'],
})
export class PurchasePaddyComponent implements OnInit {
  @Input() Type: string;
  @Input() EditID = '';
  ...
  ngOnInit() {
    this.http.getProducts(1).then((res: any) => {
      this.Bardana = res.filter(
        (x) => x.CategoryName.toLowerCase().includes('bardana')
      );
      this.Products = res.filter(
        (x) => x.CategoryName.toLowerCase().includes('paddy')
      );
    });
  ...
  public SaveData() {
    ...
    this.data.getAll().then((res1) => {
      ...
      this.http.postTask('purchasepaddy' + InvoiceID, this.purchase).then(
        () => {
          this.Cancel();
  ...
  CalculateIt() {
    this.purchase.GrossWeight =
      this.purchase.LoadedWeight - this.purchase.EmptyWeight;
    this.purchase.NetWeight =
      this.purchase.LoadedWeight -
      this.purchase.EmptyWeight -
      this.purchase.BardanaWeight +
      this.purchase.Katla;
    this.purchase.Amount = this.RoundIt(
      (this.purchase.NetWeight * this.purchase.RateOfMon) / 40
    );
    this.purchase.NetAmount = this.RoundIt(
      this.purchase.Amount -
        (this.purchase.Amount * this.purchase.TaxRatio) / 100
    );
  }
}
