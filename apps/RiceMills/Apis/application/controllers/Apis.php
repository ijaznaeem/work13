...
class Apis extends REST_Controller
{
    ...
    public function cashreport_get()
    {
        ...
        $acct = $this->db->query(
            ...
            " Select Date, 'Cash Sale' as Type, CustomerName, Address, 0 as Debit, AmountRecvd as Credit from qryinvoices where  AmountRecvd > 0 and DtCr = 'CR' AND  $filter and BusinessID = $bid UNION ALL " .
            " Select Date, 'Cash Sale Return' as Type, CustomerName, Address,  AmountRecvd as Debit, 0 as Credit from qryinvoices where  AmountRecvd > 0 and DtCr = 'DT' AND $filter  and BusinessID = $bid "
        );
    ...
    public function GetVouchNo($type = '')
    {
        $vno = $this->db->query("SELECT CONCAT('$type',
                '-', 
                LPAD(IFNULL( MAX(CAST(RIGHT(VoucherID,8) AS UNSIGNED)),0)+1 ,8,'0')) as vno FROM vouchers
                where VType = '$type'
    ...
    public function getvno_get($vtype)
    {
        $this->response(
            array('vno' => $this->GetVouchNo($vtype)), REST_Controller::HTTP_OK
        );
    }


    public function closemonth_post()
    {
        $post_data['BusinessID'] = $this->post('BusinessID');

        $this->db->query("INSERT INTO monthlybalance(Date, MonthNo, AccountID, Balance, BusinessID)
     Select CurDate(), MONTH(CURDATE()), CustomerID, Balance, BusinessID from qrycustomers where BusinessID = " . $post_data['BusinessID']);
         $this->CreateClVouchers(1, $post_data['BusinessID']);
        $this->response(array(['result' => 'success', 'message' => 'Month is closed']), REST_Controller::HTTP_OK);
    }

    private function CreateClVouchers($t, $bid)
    {
        $vouchers = $this->db->query("SELECT * from monthlybalance where BusinessID = $bid and status = 0")->result_array();
        foreach ($vouchers as $vouch) {

            $vouch['VType'] = $vouch['Balance'] * $t < 0 ? 'CP' : 'CR';
            $vouch = array(
                'Date' => date('y-m-d'),
                'CustomerID' => $vouch['AccountID'],
                'Description' => 'Month Closing Voucher',
                'Debit' => $vouch['Balance'] * $t < 0 ? abs($vouch['Balance']) : 0,
                'Credit' => $vouch['Balance'] * $t >= 0 ? abs($vouch['Balance']):0,
                'RefID' => $vouch['ID'],
                'IsPosted' => 0,
                'FinYearID' => 0,
                'RefType' => 7,
                'VoucherID' => $this->GetVouchNo($vouch['VType']),
                'VType' => $vouch['VType'],
                'ClosingID' => 0,
                'BusinessID' => $bid,
            );
