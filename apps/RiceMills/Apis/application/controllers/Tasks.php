<?php
    ...
    class Tasks extends REST_Controller
    {
        ...
        public function purchasegrain_post($id = null)
        {
            if (! $this->checkToken()) {
                $this->response(
                    [
            ...
            $pinvoice['SupplierID']    = $post_data['CustomerID'];
            $pinvoice['ProductID']     = $post_data['ProductID'];
            $pinvoice['LoadedWeight']  = $post_data['LoadedWeight'];
            $pinvoice['EmptyWeight']   = $post_data['EmptyWeight'];
            $pinvoice['BardanaWeight'] = $post_data['BardanaWeight'];
            $pinvoice['Katla']         = $post_data['Katla'];
            $pinvoice['RateOfMon']     = $post_data['RateOfMon'];
            $pinvoice['TaxRatio']      = $post_data['TaxRatio'];
            $pinvoice['BusinessID']    = $post_data['BusinessID'];

            $this->db->trans_begin();
            if ($id == null) {
                $this->db->insert('purchasegrain', $pinvoice);
                $invID = $this->db->insert_id();
            } else {
                $this->db->where('PurchaseID', $id);
                $this->db->update('purchasegrain', $pinvoice);
                $this->db->query("DELETE FROM `purchasedetails` WHERE `PurchaseID`=" . $id);
                $invID = $id;
            }
            $details = $post_data['details'];
            ...
            foreach ($details as $value) {
                $pdetails['ProductID']  = $value['ProductID'];
                $pdetails['Qty']        = $value['Qty'];
                $pdetails['PurchaseID'] = $invID;
                $pdetails['BusinessID'] = $post_data['BusinessID'];
                $this->db->insert('purchasedetails', $pdetails);
            }
            $this->db->trans_commit();
            $this->response(['id' => $invID], REST_Controller::HTTP_OK);
        ...
        public function purchasepaddy_post($id = null)
        {
            if (! $this->checkToken()) {
                $this->response(
                    [
        ...
        public function sale_post($id = null)
        {
            ...
            $invoice['CustomerName'] = isset($post_data['CustomerName']) ? $post_data['CustomerName'] : '';
        ...
        public function production_post($id = null)
        {
            ...
            $this->db->trans_begin();
            if ($id == null) {
                $this->db->insert('production', $invoice);
                $invID   = $this->db->insert_id();
                $details = $post_data['details'];
            } else {
                $this->db->where('ProductionID', $id);
                $this->db->update('production', $invoice);
                $invID   = $id;
                $details = $post_data['details'];
        ...
        public function postvouchers_post($id)
        {
            ...
            $this->response(['msg' => 'Voucher is posted'], REST_Controller::HTTP_OK);
        }

        private function PostVouchers($id = '', $bid = 0)
        {
            if ($id != '') {
                $this->db->where('VoucherID', $id);
            } else {
        ...
        public function UpdateStock(
            ...
            try {
                if ($a == 1) {
                    ...
                    $this->db->where('ProductID', $pid);
                    $this->db->where('BusinessID', $bid);
                    $this->db->update('products', ['SPrice' => $sprice]);
                } else {
        ...
        public function postproduction_post($InvoiceID)
        {
            $this->PostProduction($InvoiceID);
        ...
        public function postgrain_post($InvoiceID)
        {
            $this->PostPurchasesGrain($InvoiceID);
            $this->response(['msg' => 'Invoice Posted'], REST_Controller::HTTP_OK);
        }
        ...
        private function PostProduction($id = 0, $bid = 0)
        {
            ...
            $InvoiceRes = $this->db->get('qryproduction')->result_array();
            $this->db->trans_begin();
            if (count($InvoiceRes) > 0) {
                foreach ($InvoiceRes as $InvoiceValue) {
                    $this->db->where('ProductionID', $InvoiceValue['ProductionID']);
                    $InvoiceDetailsRes = $this->db->get('qryproductiondetails')->result_array();
                    ...
                    $this->UpdateStock(
                        ...
                        $InvoiceValue['PPrice'],
                        $InvoiceValue['SPrice'],
                        $InvoiceValue['TotKgs'], //out
                        0,                       //int
                        $InvoiceValue['Packing'],
                        '',
                        '',
                        $InvoiceValue['ProductionID'],
                        3,
                        $InvoiceValue['Date'],
                        $InvoiceValue['BusinessID']
                    );
                    ...
                    if ($PInvoiceValue['DtCr'] == 'CR') {
                        ...
                        foreach ($PInvoiceDetailsRes as $PInvoiceDetailsvalue) {
                            $this->UpdateStock(
                                ...
                                '', $PInvoiceDetailsvalue['InvoiceID'],
        ...
        private function PostPurchasesGrain($id = 0, $bid = 0)
        {
            if ($id > 0) {
                $this->db->where('PurchaseID', $id);
            } else {
            ...
            $this->db->where('IsPosted', 0);
            $this->db->where("Date <> '0000-00-00'");
            $PInvoiceRes = $this->db->get('qrypurchasegrain')->result_array();
            $this->db->trans_begin();
            if (count($PInvoiceRes) > 0) {
                foreach ($PInvoiceRes as $PInvoiceValue) {

                    $data['CustomerID']  = $PInvoiceValue['SupplierID'];
                    $data['Date']        = $PInvoiceValue['Date'];
                    $data['Credit']      = $PInvoiceValue['NetAmount'];
                    $data['Description'] = 'Bill No ' . $PInvoiceValue['PurchaseID'];
                    $data['RefID']       = $PInvoiceValue['PurchaseID'];
                    $data['RefType']     = 5;
                    $data['BusinessID']  = $PInvoiceValue['BusinessID'];
                    $data['Debit']       = 0;
                    $this->AddToAccount($data);
                    ...
                    $this->UpdateStock(
                        1,
                        $PInvoiceValue['ProductID'],
                        0,
                        0,
                        0,
                        $PInvoiceValue['NetWeight'],
                        1,
                        '', '',
                        $PInvoiceValue['PurchaseID'], 1,
                        $PInvoiceValue['Date'],
                        $PInvoiceValue['BusinessID']
                    );

                    $this->db->where('PurchaseID', $PInvoiceValue['PurchaseID']);
                    $PInvoiceDetailsRes = $this->db->get('purchasedetails')->result_array();
                    ...
                    $posted['IsPosted'] = '1';
                    $this->db->where('PurchaseID', $PInvoiceValue['PurchaseID']);
                    $this->db->update('purchasegrain', $posted);
                }
            }
            $this->db->trans_commit();
        }
