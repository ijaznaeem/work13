...
// echo $post_data['message']; exit(0);
    ...
    public function SaveVouchers_post($VouchNo =''){
      $post_data = [
        'Date' => $this->post('Date'),
        'AcctType' => $this->post('AcctType'),
        'CustomerID' => $this->post('CustomerID'),
        'Description' => $this->post('Description'),
        'Debit' => $this->post('Debit'),
        'Credit' => $this->post('Credit'),
        'RefID' => $this->post('RefID'),
        'RefType' => $this->post('RefType'),
        'FinYearID' => $this->post('FinYearID'),
        'IsPosted' => $this->post('IsPosted'),
        'CashTypeID' => $this->post('CashTypeID'),
        'Tmr' => $this->post('Tmr'),
        'DayNo' => $this->post('DayNo'),
        'VouchNo' => $this->post('VouchNo'),
        'BankID' => $this->post('BankID'),
        'CheqNo' => $this->post('CheqNo'),
        'BranchCode' => $this->post('BranchCode'),
        'sRefAcct' => $this->post('sRefAcct'),
        'PaymentHead' => $this->post('PaymentHead'),
        'UserID' => $this->post('UserID'),
      ];
      $this->load->database();

      $this->db->where('VouchNo', $VouchNo);
      $query = $this->db->get('Vouchers');

      if ($query->num_rows() > 0) {
        // Update existing record
        $this->db->where('VouchNo', $VouchNo);
        if ($this->db->update('Vouchers', $post_data)) {
          $this->response(['result' => 'Success', 'message' => 'Voucher updated successfully'], REST_Controller::HTTP_OK);
        } else {
          $this->response(['result' => 'Error', 'message' => $this->db->error()], REST_Controller::HTTP_BAD_REQUEST);
        }
      } else {
        ...
        $post_data['VouchNo'] = $this->utilities->getBillNo($this->db, $voucherType, true);
        // Insert new record

        if ($this->db->insert('Vouchers', $post_data)) {
          $this->response(['result' => 'Success', 'message' => 'Voucher inserted successfully', 'id' => $this->db->insert_id()], REST_Controller::HTTP_OK);
        } else {
          $this->response(['result' => 'Error', 'message' => $this->db->error()], REST_Controller::HTTP_BAD_REQUEST);
        }
      }
