...
export class CashReceiptComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    this.vDetailForm.title = 'Cash Receipt Voucher';
    const col = FindCtrl(this.vDetailForm, 'Amount');
    if (col) {
      col.fldName = 'Credit';
      col.label = 'Amount Received';
    }
    this.settings.Columns[5].fldName = 'Credit';
  ...
  public SaveData() {
    ...
    this.voucher.RefType = enRefType.CashRecievd;
  ...
  NavigateTo(rt = '') {
    this.router.navigateByUrl('/cash/cashreceipt' + (rt != '' ? '/' + rt : ''));
  }
  NavigatorClicked(e) {
    console.log(e);

    let dt = GetDateJSON();
    let billNo: any = 'RV' + '-' + dt.year.toString().slice(2) + '000001';
    switch (e.col.label) {
      case 'First':
