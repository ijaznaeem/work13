...
import { Details, DetailsSettings, Invoice } from './cash-sale.settings';
...
export class CashSaleComponent implements OnInit, OnChanges {
  ...
  NavigatorClicked(e) {
    let dt = GetDateJSON();
    let billNo: any = 'C' + '-' + dt.year.toString().slice(2) + '000001';
    switch (Number(e.Button)) {
      ...
      case Buttons.Next:
        ...
        break;
      case Buttons.Last:
        this.http.getData('getbno/C').then((r: any) => {
          billNo = r.billno;
          this.router.navigateByUrl(this.InvoiceUrl + billNo);
        });
        break;
  ...
  NewInvoice() {
    ...
    this.router.navigateByUrl('/sale/cash');
  }
  PrintInvoice() {
