...
export class SalesummaryComponent implements OnInit {
  ...
  FilterData() {
    ...
    let filter =
      ...
      JSON2Date(this.Filter.ToDate) +
      "'";
    if (this.Filter.CatID != '') filter += ' and CategoryID=' + this.Filter.CatID


    this.http.getData('qrysalereport?flds=ProductName,sum(Qty) as Qty, sum(Amount) as Amount&groupby=ProductName&filter=' + filter).then((r: any) => {
        this.data = r;
      });
  }
  Clicked(e) {
