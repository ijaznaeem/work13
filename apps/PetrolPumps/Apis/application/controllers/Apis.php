...
class Apis extends REST_Controller
{
  ...
  public function profitreport_get($dte1, $dte2)
  {
    ...
    $this->load->database();
    $query = $this->db->query("SELECT ProductName,  Sum(Qty) as QtySold, SUM(Amount) as Amount, SUM(Cost) as Cost, Sum(Amount-Cost) as Profit FROM qrysalereport WHERE Date BETWEEN '$dte1' AND '$dte2' GROUP BY  ProductName Order by ProductName ")->result_array();
    $this->response($query, REST_Controller::HTTP_OK);
  }
  ...
  public function daybook_get($date)
  {
    $this->output->set_header('Last-Modified: ' . gmdate("D, d M Y H:i:s") . ' GMT');
    $this->output->set_header('Cache-Control: no-store, pauno-cache, must-revalidate, post-check=0, pre-check=0');
    $this->output->set_header('Pragma: no-cache');
    $this->output->set_header("Expires: Mon, 26 Jul 1997 05:00:00 GMT");

    $bid = $this->get('bid');

    $sQuery = " SELECT 'Opening Cash' SaleType, 'Cash' Account, 'Opening Cash' as Description,  0 as Qty, 0 as Rate,  OpeningAmount as Income, 0 as Expense, 0 as Type, 1 as IsPosted, 0 InvoiceID FROM closing where Date = '$date' and BusinessID= $bid";
    $sQuery .= " UNION ALL SELECT case when Type =1 then  'Oil Sale' else 'Lubricant Sale' end  SaleType, CustomerName , ProductName,  Qty, SPrice as Rate,  Amount as Income, 0 as Expense, Type, IsPosted, InvoiceID FROM qrysalereport where DtCr= 'CR' and Date = '$date' and BusinessID= $bid";
    $sQuery .= " Union All SELECT 'Sale Return' SaleType, CustomerName , ProductName,  Qty, SPrice as Rate,  0 as Income, abs(Amount) as Expense, 3 as Type, IsPosted, InvoiceID FROM qrysalereport where DtCr= 'DT' and Date = '$date' and BusinessID= $bid";
    $sQuery .= " Union All SELECT 'Credit Sale' SaleType, CustomerName, Description,  Qty,  Rate,  0 as Income, Debit as Expense, 6, IsPosted, VoucherID FROM qryvouchers where RefType = 6 and Date = '$date' and BusinessID= $bid";
    $sQuery .= " Union All SELECT 'Cash' as SaleType, CustomerName, Description,   0, 0 as Rate,  Credit as Income, Debit as Expense, 4, IsPosted, VoucherID FROM qryvouchers where RefType <> 6 and  Date = '$date' and BusinessID= $bid";
    $sQuery .= " Union All SELECT 'Expense' as SaleType, HeadName,  Description,  0, 0 as Rate,  0 as Income, Amount as Expense, 5, IsPosted, ExpendID FROM qryexpenses where  Date = '$date' and BusinessID= $bid";
