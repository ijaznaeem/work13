...
export class SalesummaryComponent implements OnInit {
  ...
  FilterData() {
    ...
    if (this.Filter.ProductID && this.Filter.ProductID != '') {
      ...
      this.http
        .getData(
          'qrysalereport?flds=InvoiceID,ProductName,SPrice,' +
            'sum(Qty) as Qty, sum(Amount) as Amount&groupby=InvoiceID,' +
      ...
      this.http
        .getData(
          'qrysalereport?flds=ProductName,SPrice,sum(Qty) as Qty, ' +
            'sum(Amount) as Amount&groupby=ProductName,SPrice&filter=' +
            filter
        )
        .then((r: any) => {
          this.sting = JSON.parse(JSON.stringify(this.setting));
