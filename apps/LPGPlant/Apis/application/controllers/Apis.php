...
class Apis extends REST_Controller
{
    ...
    public function stockbydate_post()
    {
        ...
        $result = $this->dbquery("CALL getStockByDates ('$fromDate','$toDate', 1, $bid) ");
    ...
    public function dailystock_post()
    {
        ...
        $result = $this->db->query("
          SELECT CategoryName, SUM(Stock) as Stock
          FROM qrystock
          WHERE 1
    ...
    public function productbystore_get($storeID)
    {
        $this->output->set_header('Last-Modified: ' . gmdate("D, d M Y H:i:s") . ' GMT');
        $this->output->set_header('Cache-Control: no-store, no-cache, must-revalidate, post-check=0, pre-check=0');
        $this->output->set_header('Pragma: no-cache');
        ...
        $result = $this->dbquery("select
            ...
            Packing,
            Value as UnitValue,
            (select Stock from stock where stock.ProductID = qryproducts.ProductID and stock.StoreID = $storeID limit 1) as Stock
            from qryproducts where (BusinessID = $bid)");
...
public function salarysheet_post()
 ...
 public function dailysheet_post($date = '')
{
    if (! $this->checkToken()) {
        $this->response(
            [
    ...
    if ($fromDate == '' || $toDate == '') {
        $this->response(['status' => 'Error', 'message' => 'No date'], REST_Controller::HTTP_BAD_REQUEST);
    }

    // First procedure
    $q1   = $this->db->query("CALL sp_GetCashbookHistory('$fromDate', '$toDate')");
    $cash = $q1->result_array();
    $q1->free_result();
    mysqli_next_result($this->db->conn_id);

    // Second procedure
    $q2    = $this->db->query("CALL sp_GetStockHistoryByCategory('$fromDate', '$toDate')");
    $stock = $q2->result_array();
    $q2->free_result();
    mysqli_next_result($this->db->conn_id);

    $this->response(['cash' => $cash, 'stock' => $stock], REST_Controller::HTTP_OK);
}
