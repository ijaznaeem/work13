...
class Apis extends REST_Controller
{
    ...
    public function index_post($table = "", $id = null)
    {
        ...
        if ($this->post('BusinessID') == "") {

            $this->response(array('result' => 'Error', 'message' => 'No Business is mentioned'), REST_Controller::HTTP_BAD_REQUEST);
            return;
        }
        ...
        if ($this->db->table_exists($table)) {
          ...
          $this->db->where($this->getpkey($table), $id);
            ...
            if (count($res) > 0) {
              $row = $res[0];
              if ($row['BusinessID'] == 0) {
                $this->response(array('result' => 'Error', 'message' => 'Cannot delete record with BusinessID = 0'), REST_Controller::HTTP_BAD_REQUEST);
                return;
              }
            } else {
              $this->response(array('result' => 'Error', 'message' => 'Record not found'), REST_Controller::HTTP_NOT_FOUND);
              return;
            }
            $this->db->trans_start();
            $this->db->where($this->getpkey($table), $id);
  ...
  public function printbill_get($invID)
  {
    ...
    if (count($res) > 0) {
      $det = $this->db->where(array('InvoiceID' => $invID))
        ->select('ProductName, Qty, Pcs, SPrice, Amount, DiscRatio, Packing, Discount, NetAmount')
