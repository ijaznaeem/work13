...
Filter = {
  ...
  FindINo(){
    ...
    this.http.getData('qryinvoices?filter=InvoiceID=' + this.Filter.INo).then((r:any)=>{
      if (r.length>0){
        swal({
          ...
          this.http.postTask('makereturn', { InvoiceID: r[0].InvoiceID }).then((r: any) => {

            swal('Return!', 'Return Invoice have been created. Invoice # ' + r.id, 'success');
            this.router.navigateByUrl('/sale/salereturn/' + r.id)
