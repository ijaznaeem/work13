  ...
  ProductSelected($event) {
    ...
    if ($event.itemData) {
      ...
      if (this.sale.CustomerID) {
        this.http.getData('qrysalereport?flds=Date,SPrice,TotPcs,DiscRatio,SchemeRatio&orderby=InvoiceID Desc&limit=1&filter=CustomerID=' + this.sale.CustomerID
          + ' and ProductID = ' + this.sdetails.ProductID).then((r: any) => {
            console.log(r);
            if (r.length > 0) {
              this.LastPrice = r[0]
            } else {
