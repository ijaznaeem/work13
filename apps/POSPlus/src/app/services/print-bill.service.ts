...
export class PrintBillService {
  ...
  PrintPDFBill(data: any) {
    ...
    var imageUrl = UPLOADS_URL +  'jmart-logo.png'; // Replace this with your image URL

// Add the image to the PDF
doc.addImage(imageUrl, 'png', 4, 1, 72, 32);
    // doc.text(data.Business.BusinessName, midX, y, { align: 'center' });
    // y += 5;
    // doc.text(data.Business.Address + ', ' + data.Business.City, midX, y, {
    //   align: 'center',
    // });
    // doc.setFontSize(10);
    // y += 5;
    // doc.text('Junaid Raza: 0320-6059119', midX, y, { align: 'center' });
    // y += 5;
    // doc.text('Hasnain Raza: 0341-6554429', midX, y, { align: 'center' });
    // y += 8;
    ...
    doc.setFontSize(12);
    doc.text('We are here', x + 2, y + 6);
    doc.text('For You', x + 2, y + 12);
    ...
    doc.text('Tax: ', x + 30, y);
    doc.text(RoundTo(data.GSTAmount, 2) + '', this.billW - x, y, {
      align: 'right',
    });
    y += 6;
    doc.setFontSize(12);
    ...
    if (data.Type == 2) {
      ...
      doc.text(
        RoundTo(data.PrevBalance * 1 + data.NetAmount * 1, 0) + '',
        this.billW - x,
        y,
        {
          align: 'right',
      ...
      doc.text(
        RoundTo(
          data.AmountRecvd > data.NetAmount
            ? data.AmountRecvd - data.NetAmount
            : 0,
          0
        ) + '',
    ...
    doc.addImage(UPLOADS_URL + 'address.png', 'png', 2, y+22, 72, 10);
    doc.autoPrint();
