...
export function GetDateJSON(dte: any = null) {
  if (!dte) dte = new Date();
  return {
...
export function GetUniqueNo() {
  const d = new Date();
  return Math.trunc(d.valueOf() / 100);
}

export function PrintBill(data: any) {}
export function isInDevMode(): boolean {
  return isDevMode();
}

export function jsonToArray(jsonArray, exclude:any= []) {
  ...
  for (i = 0; i < jsonArray.length; i++) {
    for (const key in jsonArray[i]) {
      if (jsonArray[i].hasOwnProperty(key) && !exclude.includes(key)) {
        p.push(jsonArray[i][key]);
      }
    }
    valuesArray.push(p);
    p = [];
