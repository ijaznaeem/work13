...
export class HttpBase {
  ...
  getProducts() {
    ...
    return this.getData('products', { filter: filter, orderby: 'SortNo' });
  }
  getRawItems() {
    let filter = '';
    filter = 'StatusID=1 ';
  ...
  getRawStock(storeid = 0) {
    let filter = '1';
    if (storeid > 0) {
      filter += ' and StoreID =' + storeid;
    }
    return this.getData('qryrawstock', { filter: filter, orderby: 'SortNo' });
  }
  getMixStock(storeid = 0) {
    let filter = '1';
    if (storeid > 0) {
      filter += ' and StoreID =' + storeid;
    }
