...
export class SalesummaryComponent implements OnInit {
  ...
  FilterData() {
    ...
    this.http.getData("qrysalereport?flds=ProductName, sum(CASE WHEN DtCr='CR' THEN TotPcs ELSE 0 end) Sale, " +
              " sum(CASE WHEN DtCr='DT' THEN TotPcs ELSE 0 end) `Return`, sum(TotPcs) as Qty, sum(Amount) as Amount, "
