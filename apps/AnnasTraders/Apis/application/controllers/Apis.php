...
class Apis extends REST_Controller
{
    ...
    public function customeracctdetails_get($date1, $date2, $customerid)
    {
        ...
        for ($i = 0; $i < count($result); $i++) {
            ...
            if ($result[$i]['RefID'] > 0 && $result[$i]['RefType'] == 2 && $result[$i]['Credit'] > 0) {
                $details = $this->dbquery("
                SELECT ProductName, TotKgs, Sprice, Amount  from qrypinvoicedetails  where  InvoiceID = " . $result[$i]['RefID']);
                $result[$i]['Details'] = $details;
            }
        }
