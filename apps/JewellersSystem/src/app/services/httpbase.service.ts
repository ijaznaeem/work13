...
export class HttpBase {
  ...
  getProducts(CatID = 0) {
    ...
    return this.getData(
      'Products?flds=ProductID as ProductID,ProductName as ProductName&' +
        filter +
        '&orderby=ProductName'
    );
  }
  getStock(StoreID = '') {
    ...
    return this.getData('qryStock', { filter: filter, orderby: 'ProductName' });
  }
  ...
  getAcctstList(type = '') {
    ...
    return this.getData(
      'Customers?flds=CustomerName as CustomerName,Balance,GoldBalance,Address,CustomerID as CustomerID&orderby=CustomerName' +
        filter
    );
  }
  getTransporters() {
