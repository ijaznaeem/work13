...
import { ChandiTrForm } from './chandi-transaction.settings';

@Component({
  selector: 'app-chandi-transaction',
  templateUrl: './chandi-transaction.component.html',
  styleUrls: ['./chandi-transaction.component.scss'],
})
export class ChandiTransactionComponent implements OnInit, OnChanges {
  @Input() Type: string;
  @Input() EditID = '';
  ...
  public async Save(event) {
    if (this.modelData.CashID == null || this.modelData.CashID == 0) {
      ...
      bno = await this.http.getData(
        'getbno/CASH-' + enTransactionType.Chandi + '/1'
      );
  ...
  public Changed(event) {
    console.log(event);
    if (event.fldName == 'RawChandi' || event.fldName == 'ChandiCutting') {
    }
  }
  CalcChandiAmount() {}
  ...
  Cancel() {
    ...
    this.modelData.Type = enTransactionType.Chandi;
    this.btnsave = false;
  }
  ...
  NavigatorClicked(e) {
    ...
    switch (e.col.label) {
      ...
      case 'Last':
        this.http
          .getData('getbno/CASH-' + enTransactionType.Chandi)
          .then((r: any) => {
            billNo = r.billno;
            this.NavigateTo(billNo);
          });
