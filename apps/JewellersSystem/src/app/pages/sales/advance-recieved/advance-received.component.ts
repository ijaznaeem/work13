...
import { AdvanceTrForm } from './advance-received.settings';
...
export class AdvanceReceivedComponent implements OnInit, OnChanges {
  @Input() Type: string;
  @Input() EditID = '';
  ...
  navigateURL = '/sale/advance-receipt';
  Ino = '';
  tqty: any = '';
  ...
  LoadInvoice() {
    ...
    this.http.getData(`Advance/'${this.EditID}'`).then((r: any) => {
      if (r) {
  ...
  public async Save(event) {
    if (this.modelData.CashID == null || this.modelData.CashID == 0) {
      ...
      bno = await this.http.getData(
        'getbno/4/' + enTransactionType.AdvanceReceived
      );
  ...
  public Changed(event) {
    console.log(event);
    if (event.fldName == 'RawAdvance' || event.fldName == 'AdvanceCutting') {
    }
  }
  CalcAdvanceAmount() {}
  ...
  Cancel() {
    ...
    this.modelData.Type = enTransactionType.AdvanceReceived;
    this.btnsave = false;
  }
  ...
  NavigatorClicked(e) {
    ...
    switch (e.col.label) {
      ...
      case 'Last':
        this.http
          .getData('getbno/4/' + enTransactionType.AdvanceReceived)
          .then((r: any) => {
            billNo = r.billno;
            this.NavigateTo(billNo);
          });
