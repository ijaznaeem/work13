...
class Tasks extends REST_Controller
{
  ...
  public function posttransfer_post($id)
  {
    ...
    try {
      ...
      if (count($PInvoiceRes) > 0) {
        ...
        foreach ($PInvoiceDetailsRes as $PInvoiceDetailsvalue) {
          ...
          $this->UpdateStock(
            ...
            ($PInvoiceDetailsvalue['Qty']),
            0, $PInvoiceDetailsvalue['Packing'],
            '',
            '',
            $PInvoiceValue['TransferID'],
            4,
      ...
      $this->db->trans_commit();
      $this->response(array('id' => $PInvoiceValue['TransferID']), REST_Controller::HTTP_OK);
    } catch (\Throwable $th) {
  ...
  private function PostSales($id = 0, $bid = 0)
  {
    ...
    $InvoiceRes = $this->db->get($this->Invoices_model->qryinvoices_txt)->result_array();
    $this->db->trans_begin();
    if (count($InvoiceRes) > 0) {
      foreach ($InvoiceRes as $InvoiceValue) {
        $this->db->where('InvoiceID', $InvoiceValue['InvoiceID']);
        $InvoiceDetailsRes = $this->db->get($this->Invoices_model->qryinvoicedetail_txt)->result_array();
  ...
  function PostPurchases($id = 0, $bid = 0)
  {
    if ($id > 0) {
      $this->db->where('InvoiceID', $id);
    } else {
