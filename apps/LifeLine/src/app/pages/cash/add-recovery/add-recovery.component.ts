...
export class AddRecoveryComponent implements OnInit {
  ...
  VouchersList = [];
  Filter: any = {
    Date: getYMDDate(new Date(this.http.getClosingDate()))
  };
  ...
  FilterData() {
    let filter =
      "Date = '" + getYMDDate(new Date(this.Filter.Date)) + "'";
    ...
    this.http
      .getData(
        "qryinvoices?flds=CustomerID,CustomerName,Address,City,Balance as Amount,'0.0' as Recovery,  Balance&filter=" +
          filter
      )
      .then((d: any) => {
        this.VouchersList = d;
  ...
  SaveVouchers() {
    swal({
      ...
      if (willPay) {
        ...
        this.http
          .postTask('addrecovery', VocuherData)
          .then((s) => {
