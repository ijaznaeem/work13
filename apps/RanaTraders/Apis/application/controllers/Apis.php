...
class Apis extends REST_Controller
{
  ...
  public function getbno_get($type)
  {
    $bid = $this->get('bid');
    $this->db->select_max('InvoiceID');
      ...
      if ($query->num_rows() > 0) {
        $row = $query->row();
        $maxInvoiceID = $row->InvoiceID == null ? $type * 100000000 : $row->InvoiceID; // Retrieve the maximum InvoiceID

      } else {
        $maxInvoiceID = $type * 100000000;

      }
      $this->response(array('billno' =>$maxInvoiceID), REST_Controller::HTTP_OK);
  }
  public function getpbno_get()
  {
    $bid = $this->get('bid');
    $this->db->select_max('InvoiceID');

      $this->db->where('BusinessID', $bid);
      $query = $this->db->get('pinvoices');

      if ($query->num_rows() > 0) {
        $row = $query->row();
        $maxInvoiceID = $row->InvoiceID == null ? 100000000 : $row->InvoiceID; // Retrieve the maximum InvoiceID

      } else {
        $maxInvoiceID =  100000000;
