...
export class PrintBillService {
  ...
  PrintPDFBill(data: any) {
    ...
    y += 6;
    doc.text('Previous: ', x + 26, y);

    y += 15;
    doc.setFontSize(10);
    doc.setFont('Helvetica', 'normal');
    if (data.Notes != '') {
      const textWidth = doc.getTextWidth('Note: ' + data.Notes);
      doc.setFillColor(99, 99, 99);
      doc.rect(x, y - 5, 65, 8, 'F');


      doc.setTextColor(255, 255, 255);


      doc.text('Note: ' + data.Notes, x + 2, y);
    ...
    doc.text('In case of any difference, ', x + 1, y);
    doc.text('Please contact within one day', x + 1, y + 4);
