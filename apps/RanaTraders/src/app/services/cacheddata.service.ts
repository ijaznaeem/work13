...
public apiStock$ = this.http.get<any[]>(
  this.api +
  'qrystock?orderby=ProductName&bid=' +
...
public Stock$ = this._stockData$.pipe(
  ...
  //------ customers list
  private _account$ = new BehaviorSubject<void>(undefined);
  public apiAccounts$ = this.http.get<any[]>(this.api + 'customers?flds=CustomerName,Address,Balance,CustomerID&orderby=CustomerName&bid=' + this.http2.getBusinessID());

  public Account$ = this._account$.pipe(
