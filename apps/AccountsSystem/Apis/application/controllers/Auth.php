...
class Auth extends REST_Controller
{
    ...
    public function login_post()
    {
        ...
        if ($User) {
            // var_dump($User);
            $fin = $this->db->query("select * from finyears where BusinessID = $BusinessID order by FinYearID DESC limit 1 ")->result_array();
            if (count($fin) > 0) {
                $date                = date('Y-m-d');
                $data['Date']        = $date;
                $output              = $this->CreateOutPut($User[0]);
                $output['FinYearID'] = $fin[0]['FinYearID'];
                $output['BusinessID'] = $fin[0]['BusinessID'];
                ...
                $this->response(array('status' => 'false', 'msg' => 'No Finacial Year Found'), REST_Controller::HTTP_NOT_FOUND);
            }
        } else {
