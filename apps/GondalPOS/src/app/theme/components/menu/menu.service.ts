...
export class MenuService {
  ...
  public createMenu(menu: Array<Menu>, nativeElement, type) {
    let gid = JSON.parse(localStorage.getItem('currentUser')).rights;
    if (gid == null) {
      gid = '0';
    }
    const mmenu = menu.filter(m => m.groupid.includes(parseFloat(gid)));
    if (type === 'vertical') {
      this.createVerticalMenu(mmenu, nativeElement);
    }
    if (type === 'horizontal') {
