...
export class DayBookComponent implements OnInit {
  ...
  FilterData() {
    ...
    this.http.getData('qryinvoices?orderby=InvoiceID DESC' + '&filter=' + filter ).then((r: any) => {
      this.data = r;
    });
    this.http.getData('qrypinvoices?orderby=InvoiceID DESC' + '&filter=' + filter ).then((r: any) => {
      this.pdata = r;
    });
    ...
    if (e.action === 'delete') {
      ...
      if (e.data.IsPosted === '0') {
        ...
        swal({
          ...
          .then(willDelete => {
            if (willDelete) {
              this.http.postTask('delete', table).then(r => {
                this.FilterData();
      ...
      if (e.data.IsPosted === '0') {
        if (this.nWhat === '1') {
          ...
          this.router.navigateByUrl('/purchase/purchase/' + e.data.InvoiceID);
        } else if (this.nWhat === '3') {
           console.log(e.data);
          if (e.data.Credit > 0 ){
      ...
      swal({
        ...
        .then(willPay => {
          if (willPay) {
            this.http.postTask('payinvoice', { InvoiceID: e.data.InvoiceID, Amount: e.data.Balance }).then(r => {
              this.FilterData();
      ...
      swal({
        ...
        .then(willPay => {
          if (willPay) {
            ...
            this.http.postTask('payinvoice', { InvoiceID: e.data.InvoiceID, Amount: willPay }).then(r => {
              this.FilterData();
              swal('Paid!', 'Your data has been paid!', 'success');
      ...
      swal({
        ...
        this.http.postTask('makereturn', { InvoiceID: e.data.InvoiceID }).then((r: any) => {
          this.FilterData();
  ...
  CloseAccounts() {
    swal({
      ...
      if (close) {
        this.http.postTask('CloseAccount', { ClosingID: this.http.getClosingID() }).then(r => {
