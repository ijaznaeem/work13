...
export class PurchaseInvoiceComponent implements OnInit {
  ...
  getCustomers(acctType) {
    this.http.getCustList(acctType).then((res) => {
      this.Accounts = res;
    });
  }
  ...
  getproducts() {
    this.http
      .getData(
        "qryproducts?flds=ProductID,ProductName,CompanyName,SPrice,PPrice"
      )
      .then((res: any) => {
        this.Prods = res;
      ...
      this.http
        ...
        .then((res1: any) => {
          if (res1.length > 0) {
            ...
            this.selectedProduct.Stock = res1[0].Stock;
