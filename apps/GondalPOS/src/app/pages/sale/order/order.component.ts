...
import { SaleDetails, SaleModel, Order } from '../sale.model';
...
export class OrderComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.http.getData('routes').then(res => {
            this.Routes = res;
        });
        this.http.getData('salesman').then((res: any) => {
            this.Salesman = res;
        });
        this.getCustomers('');
        this.getproducts();
        ...
        if ((this.EditID && this.EditID !== '')) {
            ...
            this.http.getData('invoices/' + this.EditID).then((r: any) => {
                ...
                if (r.IsPosted === '0') {
                    ...
                    this.http.getData('qryinvoicedetails?filter=invoiceid=' + this.EditID).then((rdet: Order[]) => {
    ...
    getproducts() {
        this.http.getData('qrystock').then((res: any) => {
            this.Prods = res;
        });
    }
    AddToDetails(ord: Order) {
        ...
        const obj = {
            ...
            Bonus: ord.Bonus,
            Pcs: ord.Pcs,
            Amount: (ord.SPrice * ord.Qty),
            ProductID: ord.ProductID,
            StockID: ord.StockID,
            DiscRatio: ord.DiscRatio,
            SchemeRatio: ord.SchemeRatio,
            Discount: (ord.SPrice * ord.Qty) * ord.DiscRatio / 100,
            Scheme: ord.Qty * ord.SchemeRatio,
            BusinessID: this.http.getBusinessID(),
            NetAmount: (ord.SPrice * ord.Qty) -
                ((ord.SPrice * ord.Qty) * ord.DiscRatio / 100) - ord.Qty * ord.SchemeRatio,
        };
        console.log(obj);
    ...
    public onEdit(event) {
        ...
        event.newData.NetAmount = event.newData.Amount - event.newData.Discount - (event.newData.Qty ) * event.newData.SchemeRatio;
    ...
    SetPcs() {
        this.sdetails.SPrice = this.sdetails.PackRate ;
    }
    public calculation() {
        this.sale.Amount = 0;
        ...
        this.data.getAll().then(d => {
            ...
            for (i = 0; i < d.length; i++) {
                ...
                this.sale.Scheme += (d[i].Qty ) * d[i].SchemeRatio;
            }
        });
    }
