...
//------ invoice types list
private _InvoiceTitles$ = new BehaviorSubject<void>(undefined);
public apiInvoiceTitles$ = this.http.get<any[]>(this.api + 'invoicetitles?bid=' + this.http2.getBusinessID());

public InvoiceTitles$ = this._InvoiceTitles$.pipe(
  switchMap(() => this.apiInvoiceTitles$)
   , shareReplay(1)
);
  //------ salesman list
  ...
  public updateInvoiceTitles() {
    this._InvoiceTitles$.next();
  }
}
