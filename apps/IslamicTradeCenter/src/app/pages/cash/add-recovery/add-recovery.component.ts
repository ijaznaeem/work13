...
export class AddRecoveryComponent implements OnInit {
  ...
  public CashForm = {
    ...
    columns: [
      ...
      AddInputFld('InvDiscount', 'Discount', 2, true, 'number', {
        readonly: true,
      }),
      ...
      AddInputFld('Discount', 'Discount', 2, true, 'number'),
  ...
  SaveVouchers() {
    if (this.form.IsFormValid()) {
      ...
      if ((this.model.Credit *1 +  this.model.Discount* 1) >  this.model.InvBalanceAmount){
  ...
  async FindINo(InvoiceNo) {
    const Invoice: any = await this.http.getData(
      'qryinvoices?filter=InvoiceID=' + InvoiceNo
    );
    if (Invoice.length > 0) {
