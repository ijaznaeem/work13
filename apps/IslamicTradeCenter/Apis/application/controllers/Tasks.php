...
class Tasks extends REST_Controller
{
    ...
    public function sale_post($id = null)
    {
        ...
        $invoice['TitleID']     = $post_data['TitleID'];
    ...
    public function makereturn_post()
    {
        ...
        $this->db->query("INSERT INTO invoices (InvoiceID, Date, CustomerID, Amount, Discount,
        ExtraDisc, Scheme, PrevBalance, AmountRecvd, DtCr, Type, Notes, IsPosted, FinYearID,  SalesmanID,
        BusinessID, UserID, RouteID, ReturnID, TitleID )
        select 0," . $data['Date'] . ", CustomerID, Amount, Discount,
        ExtraDisc, Scheme, PrevBalance, AmountRecvd, 'DT', Type, Notes, 0, FinYearID, SalesmanID,
        BusinessID, UserID, RouteID, " . $data['InvoiceID'] . ", TitleID from invoices where InvoiceID = " . $data['InvoiceID']);

        $ID = $this->db->insert_id();
        $this->db->query("INSERT INTO invoicedetails(
        InvoiceID, ProductID, Qty, Pcs, Packing, SPrice, PPrice, StockID, StoreID, DiscRatio, GSTRatio,
        SchemeRatio, Bonus, Discount, RateDisc, Remarks, BusinessID)
        select   $ID, ProductID, Qty, Pcs, Packing, SPrice, PPrice, StockID, StoreID, DiscRatio, GSTRatio,
        SchemeRatio, Bonus, Discount, RateDisc, Remarks, BusinessID from invoicedetails where InvoiceID = " . $data['InvoiceID']);


      $this->db->query('Update Invoices Set ReturnID =' . $ID . ' Where InvoiceID = ' . $data['InvoiceID'] );
        $this->db->trans_commit();
        $this->response(array('id' => $ID), REST_Controller::HTTP_OK);
    }
    ...
    public function CloseAccount_post($close, $bid)
    {
        $post_data = $this->post();
        try {
            ...
            $this->db->trans_commit();
            if ($close == 1) {$data1['Status'] = '1';
                $data1['ClosingAmount']                     = 0;
                $this->db->where('ClosingID', $post_data['ClosingID']);
                $this->db->update('closing', $data1);

                $this->response(array('msg' => 'Account Closed'), REST_Controller::HTTP_OK);} else {
                $this->response(array('msg' => 'All Data is Posted'), REST_Controller::HTTP_OK);
            }
        } catch (\Exception $e) {
            die($e->getMessage());
        }
                ...
                if ($PInvoiceValue['DtCr'] == 'CR') {
                    ...
                    foreach ($PInvoiceDetailsRes as $PInvoiceDetailsvalue) {
                        $this->UpdateStock(
                            ...
                            $PInvoiceDetailsvalue['ExpiryDate'],
                            $PInvoiceDetailsvalue['BatchNo'],
                        ...
                        $this->db->where('ProductID',$PInvoiceDetailsvalue['ProductID'])
                        ->update('products', array(
                          "SPrice"      => $PInvoiceDetailsvalue['SPrice'],
                          "PPrice"      => $PInvoiceDetailsvalue['PPrice'],
                          "RetailRate"  => $PInvoiceDetailsvalue['RPrice'],
                        ));
