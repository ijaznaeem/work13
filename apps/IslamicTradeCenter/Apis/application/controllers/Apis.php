...
class Apis extends REST_Controller
{
  ...
  public function salerecovery_post(){
    $fromDate = $this->post('FromDate');
    $tomDate = $this->post("ToDate");
    $salesmanID = $this->post("SalesmanID");
    $companyID = $this->post("CompanyID");
    $Options = $this->post("Options");
    $bid = $this->post("BusinessID");

    $result = $this->db->query("CALL proc_sale_and_recovry('$fromDate',' $tomDate',
          $salesmanID, $companyID, $Options, $bid);")->result_array();
