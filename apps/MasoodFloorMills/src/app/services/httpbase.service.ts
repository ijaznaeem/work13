...
export class HttpBase {
  ...
  getProducts(type = 0) {
    ...
    return this.getData('qryproducts?flds=ProductID,CategoryName,ProductName,SPrice,PPrice,Packing&' + filter);
  }
  getStock(t = 0) {
    let filter = ''
    if (t != 0) {
    ...
    return this.getData('qrystock?' + filter);
  }
  getUsers() {
  ...
  getCustomers(t: string) {
    let filter = '';
    if (t !=''){
      filter = 'filter=AcctTypeID =' +t;
    }
    return this.getData('customers?orderby=CustomerName&' + filter);
  }
