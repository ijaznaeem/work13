...
export class BankPaymentComponent implements OnInit {
    ...
    this.activatedRoute.params.subscribe((params: Params) => {
      ...
      if (params.EditID) {
        ...
        this.http.getData("qryvouchers?filter=VoucherID='" + this.EditID + "'").then((r: any) => {
          this.Voucher = r[0];
          this.Voucher.VoucherID = this.EditID;
  ...
  GetVNO() {
    this.http.getData('getvno/'+ this.Voucher.VType).then((r:any)=>{
      this.Voucher.VoucherID = r.vno
    })
  }
  LoadCustomer(event) {
    if (event.AcctTypeID !== '') {
  ...
  SaveData() {
    ...
    this.http.postTask('vouchers' + voucherid, this.Voucher).then(r => {
      ...
      if (this.EditID != '') {
        this.router.navigateByUrl('/cash/bankpayment/');
      } else {
        this.IniatModel()
