...
import { GetDateJSON, GetProps, JSON2Date, RoundTo } from '../../../factories/utilities';
...
export class PurchasePaddyComponent implements OnInit {
  ...
  ngOnInit() {
    this.http.getProducts(1).then((res: any) => {
      this.Bardana = res.filter(x=> x.ProductName.toLowerCase().indexOf('bardana')>=0 );
      this.Products = res.filter(x=> x.ProductName.toLowerCase().indexOf('paddy')>=0);
    });
