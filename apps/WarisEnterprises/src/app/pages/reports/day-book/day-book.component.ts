...
import { CashSaleSetting, SaleSetting } from './sale.setting';
...
export class DayBookComponent implements OnInit {
  ...
  FilterData() {
    ...
    if (this.nWhat === '1') {
      ...
      this.settings = CashSaleSetting;
      table =
        "invoices?flds=InvoiceID,Date,'Cash Sale' as CustomerName,Amount,Discount,DtCr,IsPosted&orderby=InvoiceID DESC";
    }
    ...
    this.http.getData(table + '&filter=' + filter).then((r: any) => {
      if (this.nWhat == '4') {
        r.map((x) => {
          ...
          x.NetAmount = Number(x.Amount) - Number(x.Discount);
        });
      } else {
        r.map((x) => {
          x.IsPosted == '1' ? (x.Posted = 'Posted') : (x.Posted = 'Un Posted');
  ...
  Clicked(e) {
    ...
    if (e.action === 'delete') {
      ...
      if (e.data.IsPosted === '0') {
        ...
        } else if (this.nWhat === '1' || this.nWhat === '4') {
      ...
      if (this.nWhat === '1') {
        ...
        this.http.getData('printbill/'+ e.data.InvoiceID).then((sale:any)=>{
          sale['Business'] = this.http.GetBData();
          sale['UserName'] = this.http.getUserData().username

          this.bill.PrintPDFBill(sale);
        })
      ...
      if (e.data.IsPosted === '0') {
        if (this.nWhat === '1') {
          ...
          if (e.data.DtCr === 'CR') {
            this.router.navigateByUrl('/sales/cash-sale/' + e.data.InvoiceID);
            // this.http.openModal(CreditSaleComponent, {EditID: e.data.InvoiceID})
          }
        } else if (this.nWhat === '2') {
      ...
      if (e.data.IsPosted === '0') {
        ...
        this.http.postTask(url, {}).then((r) => {
          ...
          if (this.nWhat == '1' || this.nWhat == '2') {
            this.cachedData.updatedStock();
          }
