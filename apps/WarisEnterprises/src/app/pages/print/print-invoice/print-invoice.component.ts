          ...
          this.http
            ...
            .then((r: any) => {
              const det = Array.from(
                new Set(r.map((item) => item.CompanyName))
              ).map((g) => {
                return {
                  CompanyName: g,
                  Amount: FindTotal(r.filter((i) => i.CompanyName === g), 'Amount'),
                  Discount: FindTotal(r.filter((i) => i.CompanyName === g), 'Discount'),
                  NetAmount: FindTotal(r.filter((i) => i.CompanyName === g), 'NetAmount'),

                  values: r.filter((i) => i.CompanyName === g),
                };
              });
              let k = 0;
              det.forEach(e => {
