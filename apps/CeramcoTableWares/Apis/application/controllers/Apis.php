...
class Apis extends REST_Controller
{
    ...
    public function profitreport_get($dte1, $dte2)
    {
        ...
        $this->load->database();
        $bid   = $this->get('bid');
        $query = $this->db->query("SELECT ProductName, SUM(NetAmount)/Sum(Qty) as AvgPrice,  Sum(Qty) as QtySold, SUM(NetAmount) as Amount FROM qrysalereport WHERE Date BETWEEN '$dte1' AND '$dte2' and BusinessID = $bid  GROUP BY  ProductName Order by ProductName ")->result_array();
        $this->response($query, REST_Controller::HTTP_OK);
    }
    ...
    public function saveformulation_post($id = 0)
    {
        $ProductID = $this->post('ProductID');
        $data['BatchQty'] = $this->post('Qty');
        $data['BatchUnitID'] = $this->post('UnitID');

        $this->db->where('ProductID', $ProductID);
        $this->db->update('products', $data);

        $this->db->query("DELETE FROM formula_details WHERE ProductID = " . $ProductID);
        $details = $this->post('details');
        for ($i = 0; $i < count($details); $i++) {
            $data = array();
            $data['ProductID'] = $ProductID;
            $data['Qty'] = $details[$i]['Qty'];
            $data['RawID'] = $details[$i]['RawID'];
            $data['UnitID'] = $details[$i]['UnitID'];
            $data['BusinessID'] = $this->post('BusinessID');
            $this->db->insert('formula_details', $data);
        }

        $this->response(array('id' => $id, 'msg' => 'Formulation saved successfully'), REST_Controller::HTTP_OK);
    }
}
