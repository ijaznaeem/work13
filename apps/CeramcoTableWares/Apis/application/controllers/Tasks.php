            ...
            foreach ($InvoiceRes as $InvoiceValue) {
                ...
                if ($InvoiceValue['DtCr'] == 'CR') { // sale
                    ...
                    foreach ($InvoiceDetailsRes as $InvoiceDetailsvalue) {
                        ...
                        $data['Description'] = $InvoiceDetailsvalue['ProdcutName'];
                    ...
                    if ($InvoiceValue['Discount'] + $InvoiceValue['ExDiscount']) {
                        $data['CustomerID']  = $InvoiceValue['CustomerID'];
                        $data['Date']        = $InvoiceValue['Date'];
                        $data['Credit']      = 0;
                        $data['Debit']       = $InvoiceValue['Discount'] + $InvoiceValue['ExDiscount'];
                        $data['Description'] = 'Discount ' . $InvoiceValue['InvoiceID'];
                        $data['RefID']       = $InvoiceValue['InvoiceID'];
                        $data['RefType']     = 1;
                        $data['BusinessID']  = $InvoiceValue['BusinessID'];
                        $this->AddToAccount($data);
        ...
        if (count($PInvoiceRes) > 0) {
            foreach ($PInvoiceRes as $PInvoiceValue) {
                if ($PInvoiceValue['DtCr'] == 'CR') {
                    ...
                    if ($PInvoiceValue['Discount'] + $PInvoiceValue['ExDiscount'] > 0) {
                        $data['CustomerID']  = $PInvoiceValue['CustomerID'];
                        $data['Date']        = $PInvoiceValue['Date'];
                        $data['RefID']       = $PInvoiceValue['InvoiceID'];
                        $data['RefType']     = 2;
                        $data['SalesmanID']  = 0;
                        $data['BusinessID']  = $PInvoiceValue['BusinessID'];
                        $data['Debit']       = $PInvoiceValue['Discount'] + $PInvoiceValue['ExDiscount'];
                    ...
                    if ($PInvoiceValue['Discount'] + $PInvoiceValue['ExDiscount'] > 0) {
                        ...
                        $data['SalesmanID']  = 0;
                        $data['BusinessID']  = $PInvoiceValue['BusinessID'];
                        $data['Credit']      = $PInvoiceValue['Discount'] + $PInvoiceValue['ExDiscount'];
                        $data['Description'] = 'Discount';
                        $data['Qty']         = 0;
                        $data['Rate']        = 0;
                        $data['Debit']       = 0;
