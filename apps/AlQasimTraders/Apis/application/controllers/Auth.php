...
class Auth extends REST_Controller
{
    ...
    public function login_post()
    {
        ...
        $BusinessID = 1;     // $this->post('password');
        ...
        if (count($User)) {
          // var_dump($User);
          $res1 = $this->db->query("select * from closing where ClosingID=(select max(ClosingID) from closing) and  status = 0 ")->result_array();
          if (count($res1) > 0) {
              $res = $this->db->query("select * from closing where ClosingID=(select max(ClosingID) from closing) and Date = '".$date."' and Status = 0")->result_array();
              if (count($res) > 0) {
