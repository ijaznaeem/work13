...
@Component({
  ...
  styleUrls: ['./cash-expense.component.scss'],
})
export class ExpenseComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.LoadCustomer('');
    this.Voucher.RefType = 5; // 5 for Expense
  ...
  async FindINo() {
    ...
    if (voucher.Credit > 0)
      this.router.navigate(['/cash/expense/', this.Ino]);
    else this.router.navigate(['/cash/expense/', this.Ino]);
  }
  LoadCustomer(event) {
    this.http
      .getData(
  ...
  SaveData() {
    ...
    this.Voucher.VoucherType = 3; // 3 for Expense
    ...
    this.http
      ...
      .then((r: any) => {
        ...
        if (this.EditID != '') {
          this.router.navigateByUrl('/cash/expense/' + this.EditID);
        } else {
          this.Voucher = new VoucherModel();
          this.Voucher.RefType = 5; // Reset to Expense type
          this.Voucher.Discount = 0; // Reset Discount to 0
          this.cmbCustomer.focusIn();
          this.NavigatorClicked(Buttons.Last);
        }
      })
      .catch((err) => {
    ...
    switch (Number(e.Button)) {
      case Buttons.First:
        this.http.getData('getvouchno/P/0/F').then((r: any) => {
          this.router.navigateByUrl('/cash/expense/' + r.Vno);
        });
        break;
  ...
  Cancel() {
    this.Voucher = new VoucherModel();
    this.Voucher.RefType = 4; // Reset to Expense type
    this.Voucher.Discount = 0; // Reset Discount to 0
